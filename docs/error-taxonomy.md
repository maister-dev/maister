...
>   tree; Codex adversarial review). **(Designed — ADR-101)** Three shared-tree
...
>   the child stays in `Review`, never auto-resolved. **(Designed — ADR-101)** A
...
> closed union; **Designed — ADR-101**, supersedes ADR-099 §4's launch gate). The
