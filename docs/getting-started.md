...
receipts — ADR-164) under `<runtimeRoot>/.maister/execution-host/`,
...
- **Moving `MAISTER_WORKTREES_ROOT` without `MAISTER_WORKSPACE_ROOTS`**
  (ADR-164) — the supervisor only adopts worktrees under its
  ...
  `supervisor/.env`.
- **Copying `.maister/execution-host/` between machines** (ADR-164) — a
