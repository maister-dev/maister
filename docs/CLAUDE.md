...
| [`system-analytics/agent-mentions.md`](system-analytics/agent-mentions.md) | Agent mentions in task comments (ADR-151, Designed): write-time `@<agentId>` resolution/expansion, `agent_schedules` mention binding, summonability predicate, and the additive directed-summon branch of the `agent_triggers` consumer. |
