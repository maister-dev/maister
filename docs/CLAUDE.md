...
| [`system-analytics/execution-policy.md`](system-analytics/execution-policy.md) | Flow execution-control policy (ADR-095, Implemented): preset + 9 composable autonomy axes (A self-correction / B escalation / C output shaping), immutable launch snapshot, fail-closed resolvers, no-blind-ship guard. |
...
| [`system-analytics/local-packages.md`](system-analytics/local-packages.md) | Editable local packages (ADR-095, Designed — Flow Studio Phase C): platform-scoped git-backed working dir, session edit-lock, cut-version into `package_installs`, member-level fork/apply, fork lineage for the Phase-2 PR-back. |
