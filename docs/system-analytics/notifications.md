...
- **NTF-08:** Notification triggers MUST be `decisions` deltas and digests only, and MUST NEVER be per-event by default. The delta rides two mechanisms that are not redundant: `run.needs_input` and `run.review_opened` (migration `0167`) wake the consumer immediately, and a `system_sweep` backstop re-derives the count per tick using the SAME delta function. The events give latency; the backstop gives completeness, because a decision also opens or closes for reasons with no domain write at all — a blocking relation added or removed moves the count and emits nothing. Because the two hold separate leases, the read-compare-publish MUST be atomic per owner — two events with different ids are two notifications, which no delivery-side idempotency can collapse. `createHitlRequest` MUST be the ONLY writer of `hitl_requests` and MUST emit in the same transaction as the row it inserts: a second writer is a decision nobody is told about, so the property is enforced by a test rather than by convention.
...
- **NTF-12:** Enabling a transport MUST create its delivery TARGET and the intent together, in one transaction — a `push_subscriptions` endpoint for `web_push`, an owner-scoped `webhook_subscriptions` row carrying the SAME `event_types` for `webhook`. Fan-out requires both halves and matches on the target, so an intent alone reports success and delivers nothing, and a target whose types differ silently overrides what the reader selected.
...
- **EDGE-NTF-06:** A project-less local-package assistant run (ADR-097) MUST persist its HITL request and emit NO `run.needs_input`. `domain_events.project_id` is `NOT NULL`, so emitting aborts the transaction the HITL row was written in and the supervisor permission request is cancelled — the scratch dialog is that run's live record.
