...
- Execution-host contract (ADR-164, Designed): [`execution-hosts.md`](execution-hosts.md) — `run_sessions.execution_assignment_id` attributes each spawn to the assignment epoch that created it, and `run_sessions.host_session_id` is written by the `session.create` acknowledgement instead of a post-prompt scan.
