...
**Status:** Implemented canonical-only schema and destructive preflight guards; **Designed** complete historical byte/association import and isolated lifecycle qualification (AB-11/16). The current importer refuses ordinary Stage A logs and file locators, so an ordinary-history upgrade is not qualified.
