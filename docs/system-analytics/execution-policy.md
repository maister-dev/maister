...
- The C1 `auto_on_ready` autopilot is distinct from lane-bounded auto-promotion
  (ADR-126, Implemented): a run already governed by `promotionFromSnapshot ===
...
- (Designed — ADR-165) **Tree-budget min-merge with the orchestrator node's
  ...
  keeps deriving from the effective limit through
  `MAISTER_BUDGET_HARD_MULTIPLIER`. The disposition is unchanged
  (terminate-cascade, no escalate rung), and budget logs record
