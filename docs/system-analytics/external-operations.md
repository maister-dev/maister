...
Revocation sets `revoked_at`; no row is deleted. Post-issuance edits to `scopes`,
`name`, and `expires_at` — the `active → active` self-transitions and the
`token_lifecycle_events` ledger behind them — are owned by
[`token-lifecycle.md`](token-lifecycle.md) (ADR-168).
...
- **HITL answer refusal (P0-4 — Designed)** → `hitl_respond` preserves the
  ...
  [HITL](hitl.md#respond-refusal-reasons-p0-4--designed).
...
- Related domains: [`token-lifecycle.md`](token-lifecycle.md) (issue → edit →
  revoke/expire, the mutability matrix, the `token_lifecycle_events` ledger),
  [`flow-graph.md`](flow-graph.md) (gate_results,
