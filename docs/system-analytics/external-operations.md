...
- **`project_tokens`** — one row per issued token. Stores `prefix` (first 12
  ...
  (Implemented)
- **Agent tokens** (M33 — Implemented, ADR-088) — `token_kind='agent'` rows with
  ...
  [agents.md](agents.md).
- **New scopes** (M33 — Implemented) — `tasks:triage` (the triage verdict op),
...
- **MCP facade (`@maister/mcp`)** — standalone `mcp/` workspace package; eight
  ...
  comment routes, following the `hitl_*` idiom. (Implemented) M33 (Implemented,
  ADR-088) adds `triage_set` (the triager's verdict op over
