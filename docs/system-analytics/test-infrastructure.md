...
| Historical upgrade | M; web Vitest `integration` + real PG/peer | Existing `web/scripts/__tests__/import-legacy-execution-data-plane.integration.test.ts`, `web/lib/db/__tests__/migration-0135-canonical-data-plane-cutover.integration.test.ts`; `web/lib/db/__tests__/staged-migrate-cli.integration.test.ts` and the pure `migration-stages.test.ts` (S4.1); new forward migration suite | AT-11 CLI invocation/interrupt/readback, complete historical source preservation, unchanged guards and fresh/upgrade paths. S4.1 covers the three operator stages end to end on a real 0130 baseline: additive stops before the destructive cut-over, a drifted ledger and an out-of-order stage refuse, active legacy work blocks the association stage, and the importer refuses outside its staged window. |
