...
- **PR lifecycle state (Implemented, ADR-139)** — five workspace columns track the
...
- **Reopen (Implemented, ADR-140)** — a `Done` top-level (`parent_run_id IS NULL`)
...
- **Sync lifecycle claim (Implemented, ADR-140)** — "sync with target" is a 6th
...
- **(Implemented, ADR-140)** A `sync` lifecycle claim and a `promoteRun` promotion
