...
| **R09 / AB-09** Object intent/event/ACK convergence (OBJ-04/05) | Separate immutable identity/declarations from sealed metadata; one monotonic object reducer for HTTP evidence, canonical event and reconciliation; no current-owner mutation from stale evidence. | O; S1, S2 reducer contracts | **AT-09**, all ACK/event orders, lost ACK, replay, release and supersession; valid intent becomes available once, conflicting metadata remains quarantined. | Designed |
