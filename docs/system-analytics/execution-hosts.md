...
> state-aware command/receipt retirement). Native AB-09 is qualified in S3.1,
> AB-13 in S3.2, AB-15 in S3.3 and AB-12 in S3.4; AB-16 has a qualified S5.1/S5.2
> core; AB-14 in S3.6. AB-11/17 and the remaining AB-16 matrix stay Designed for S3–S5. Decision:
...
runner/session model ([sessions.md](sessions.md)). The supported deployment
keeps one host and loopback HTTP: no remote transport, multiple-host placement,
host UI, or remote enrollment. Repository/worktree paths remain a same-host
Stage C boundary, while the Stage B execution event/runtime-data plane is
fully path-free.

The assignment, command, receipt, and opaque workspace contracts remain this
domain's authority. Runtime event and file metadata are now owned by the
manager-side [execution event plane](execution-event-plane.md),
[prompt lifecycle](execution-prompt-lifecycle.md), and
[runtime-object plane](execution-runtime-objects.md); the bounded
[data-plane cutover](execution-data-cutover.md) removed legacy fallback.
...
The data-plane activator (`ensureLocalExecutionDataPlane()` at web startup,
lazy resolution, and every system sweep) runs the registrar and idempotently
starts canonical event ingestion after a successful registration. The
registrar reads `GET /health` and applies this policy under a
...
The resolver (`localHost()`) performs the idempotent data-plane activation —
one health call per 30 s window, single-flight under concurrency — and never
awaits a domain sweep: a
...
sequenceDiagram
    ...
    WEB->>SUP: open canonical event stream after successful registration
...
## A/B stabilization requirements

The current source at `33f9d34f4393f037c8ace935f3722ad449bac2f7` and local
`main` at `5b458457a94b689713491d158e751553d676be6a` match the
[audit baseline](../../.ai-factory/reviews/stage-ab-execution-host-audit-2026-09-05.md).
All R01–R17 remain applicable. Audit reproductions are historical evidence;
source inspection confirms the mechanisms, not newly executed acceptance.
The [implementation plan](../../.ai-factory/plans/stage-ab-stabilization.md)
contains the exact source anchors and dependency-ordered task list.
...
| **R06 / AB-06** Single terminal reconciliation (PRM-03/06) | One reducer used by live delivery, event projection, query/wait and recovery; receipt-first terminal is evidence-awaiting-canonical; exact versioned outcome identity. | C; AB-05 | **AT-06**, completed and nested-error receipt first; no terminal owner action until exact canonical agreement; disagreement poisons only the operation. | Designed |
| **R07 / AB-07** Unknown admission stays recoverable (PRM-02/03) | Explicit transport unknown state separate from command evidence and application; retry same frozen request/ID; bounded attempts then durable rearmable reconciliation, never fabricated domain failure. | C; AB-06 | **AT-07**, host accepts, ACK/receipts/SSE partitioned beyond retries, ACP completes, reconnect: one ACP effect and correct owner application. | Designed |
| **R08 / AB-08** Fence every create-derived write (Stage A E-EH-11, EVT-05) | Atomic historical evidence settlement separate from current binding application; authoritative run/assignment/logical-session lock and applied/stale result required by all post-create callers. | C; AB-06 | **AT-08**, delayed epoch N ACK and restart receipt fold after N+1 create: old evidence may settle, N+1 host/ACP session IDs and status remain unchanged. | Designed |
| **R09 / AB-09** Object intent/event/ACK convergence (OBJ-04/05) | Separate immutable identity/declarations from sealed metadata; one monotonic object reducer for HTTP evidence, canonical event and reconciliation; no current-owner mutation from stale evidence. | O; S1, S2 reducer contracts | **AT-09**, all ACK/event orders, lost ACK, replay, release and supersession; valid intent becomes available once, conflicting metadata remains quarantined. | Implemented native intent/seal reconciliation (S3.1; `runtime-object-lifecycle.integration.test.ts`, 13/13); historical import and crash-gap repair remain Designed for S4/S3.5. |
| **R10 / AB-10** State-aware command/receipt retention (PRM-12) | Durable eligibility exchange, exact terminal evidence, owner disposition, run/replay constraints and compact tombstones. Accepted/unknown/unapplied operations never age out. | C; AB-05/06/07/08 | **AT-10**, controlled time beyond seven days for accepted, unknown, unapplied and eligible operations; recovery/idempotency survive host+web restart. | Designed |
| **R11 / AB-11** Full resumable historical preservation (CUT-03/04/05/07/09) | Deterministic inventory/manifest and authorized host import, large-file chunks, real readback, locator/association CAS, five genuine lanes, guarded 0134–0136 cutover. | M; S3 | **AT-11**, full Stage A fixture, logs >25 MiB, uploads/evidence/file locators, multiple incarnations; interrupt/resume, run real migration and read history with web denied source access. | Designed |
| **R12 / AB-12** Safe untrusted downloads (OBJ-02/07) | Central safe attachment header policy on direct/proxy content paths, safe filename encoding and existing response-data authorization. No active MIME inline. | O; S0 (can start independently) | **AT-12**, real authorized browser opens HTML/SVG artifacts; no script or authenticated side effect; positive authorized download and foreign/unauthorized denial. | Implemented (S3.4: `runtime/safe-download.ts`; browser lane `pnpm --filter maister-web test:e2e:execution-ab`, 5/5) |
| **R13 / AB-13** Post-seal integrity and failure evidence (OBJ-04/07/12) | Immutable seal; no-follow descriptor validation; full hash into bounded response spool before response; missing/corrupt host state + canonical event; exact descriptor served. | O; AB-09 | **AT-13**, same-size tamper, truncate, unlink, symlink/path swap and mutation during read; no successful obsolete-digest body; correct durable missing/corrupt transition. | Implemented host verification (`runtime-objects.integration.test.ts`, 18 real HTTP/SQLite/file cases); manager body/range verification remains AB-15. |
| **R14 / AB-14** Fair retention scans (OBJ-12) | Indexed keyset/last-examined progress recorded for every examined row, claim/fence deletion and protected-reference predicate. | O; AB-09/10 | **AT-14**, >1 page protected at front, eligible later and transient deletion failures; bounded sweeps reach all eligible rows and preserve protected bytes. | Implemented (S3.6: `runtime-object-retention.ts` keyset sweep over `execution_runtime_object_retention_progress`, holds under the object row lock, `runtime-object-retention.integration.test.ts` 8/8) |
| **R15 / AB-15** Digest semantics match bytes (OBJ-06) | Content-Digest hashes actual body; Repr-Digest/ETag identify full verified object. Proxy validates each independently and actual body, with no transformation. | O; AB-13 | **AT-15**, independently hash actual 200/206 bytes, including nonzero range; slice digest differs from whole digest where appropriate. | Implemented S3.3: host and manager spool verification, real HTTP/Postgres and browser range/refusal scenarios; see the runtime-object domain contract. |
| **R16 / AB-16** Operation-scoped boundary and real lifecycle proof (CUT-09/11/12) | AST/callsite inventory plus isolated real-process qualification; static guard is supplementary. | Q; S0 topology, S1–S4 final behavior | **AT-16**, inject prohibited operation into already allowed mixed file to fail guard; production web cannot read host root, yet lifecycle/history/object scenarios succeed through real HTTP/SSE. | Implemented core (S5.1 static guard: `web/test-support/filesystem-ownership.ts` + operation-scoped inventory, 9/9 incl. three mutation cases; S5.2 core: `execution-ab-isolation.integration.test.ts` 4/4 — production `server.ts` over a fresh build under `sandbox-exec`, denied-root negative control against the live host state store, HTTP/Postgres lifecycle with upload, SIGKILL restart re-attaching to a live host session). Remaining for S5.2/S5.3: process-death/partition matrix, Linux uid/mount-namespace driver, CI wiring. |
...
| `GET /runtime-events/span?streamId&after&through` (Designed — ADR-167 D5 amendment 2026-09-23) | `streamId`, `after`, `through` are query parameters (untrusted); the current stream identity and replay floor are server-derived; no command id is accepted, so every retained envelope of the range is returned | Read-only: never ACKs, prunes or opens SSE. Pages are bounded like the SSE replay (≤ 500 rows, ≤ 1 MiB). A foreign `streamId` answers `unavailable/stream_identity_changed` and `after` below the replay floor answers `unavailable/replay_floor_lost` (200); `through` past the highest emitted sequence answers `unavailable/beyond_emitted`; `after >= through` or a malformed sequence → 409 `PRECONDITION`/`invalid_event_span`; storage failure → 503 `EXECUTOR_UNAVAILABLE`. The manager treats any non-200 as "feed unavailable" and falls back to canonical ingest. |
