...
- Execution-host contract (ADR-164, Designed): [`execution-hosts.md`](execution-hosts.md) — `recoverExecutionCommands()` runs before the resume/takeover/reconcile sweeps, the `system_sweep` pass adds the command-recovery + 7-day retention passes and the `active`-assignment-on-parked-run backstop, and the reconciler checks adopted handles read-only (`workspace-handle-lost` WARN on 404).
