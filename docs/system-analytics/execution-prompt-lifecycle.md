...
| any pending class on a host whose stream is `lost` | `Crashed` (`stream-lost`) | `Reworked`, `decision='turn_lost'` | `applied` |
| `failed {turn_lost}`, settled-unapplied, found by Recover on a still-open attempt | Recover re-dispatches one fresh prompt | the crashed attempt is closed | `superseded` |
