...
- Execution-host contract (ADR-164, Designed): [`execution-hosts.md`](execution-hosts.md) — agent `worktree` / `repo_read` / `none` workspaces adopt as `git_worktree` / `repo_checkout` / `directory` handles, and the idle `messageChildRun` branch mints a `resume` assignment.
