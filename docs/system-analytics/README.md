...
| Workbench git | [`workbench-git.md`](workbench-git.md) | Run git panel (ADR-181): one status-independent policy + lazy `git-state` read model for commit, preserve-first discard, publish under a public branch name, update onto base/target/published, PR before promotion + finalize, and worktree re-attach. |
