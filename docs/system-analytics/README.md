...
| Run trace | [`run-trace.md`](run-trace.md) | The Evidence/Trace/Ledger plane boundary: the artifact projector derives only `preview` surfaces, and every dispatched prompt is recorded once as a `user` `run_messages` row behind a per-dispatch key, sharing the transcript projector's sequence lock. |
