...
| Attention | [`attention.md`](attention.md) | The two canonical attention counters `decisions` and `updates` (ADR-168), the cross-project decision queue, the activity feed + per-user read cursor, the deterministic digest, and the user-scoped SSE stream (ADR-170). |
...
| Home navigation | [`home-navigation.md`](home-navigation.md) | Home information architecture (ADR-171): `/` renders the Desk, the portfolio moves to `/projects`, the rail re-cut and total route classifier, and the non-admin default landing route. |
...
| Notifications | [`notifications.md`](notifications.md) | User-scoped notification subscriptions and web push (ADR-172) over the widened ADR-077 engine: nullable event scope, two-axis subscription matching, and the two-phase-commit sender. |
...
| Work stages | [`work-stages.md`](work-stages.md) | Derived work-stage vocabulary (ADR-169): the pure total `deriveWorkStage` classifier, `blocked` as an attribute, and the batched cross-project read model behind `/work`. |
