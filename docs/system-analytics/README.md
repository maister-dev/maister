...
| Execution event plane | [`execution-event-plane.md`](execution-event-plane.md) | Durable manager-owned execution event ingestion, host outbox/replay/ACK, ordering, fencing, redaction, and browser projection cutover (ADR-167, Designed). |
| Execution prompt lifecycle | [`execution-prompt-lifecycle.md`](execution-prompt-lifecycle.md) | Accepted-command async prompt lifecycle, receipt/event reconciliation, serializable prompt handles, typed owners, and restart safety (ADR-167, Designed). |
| Execution runtime objects | [`execution-runtime-objects.md`](execution-runtime-objects.md) | Path-free opaque runtime-object catalog, fenced transfer/range/delete contracts, host byte ownership, and retention (ADR-167, Designed). |
| Execution data-plane cutover | [`execution-data-cutover.md`](execution-data-cutover.md) | Immutable per-run data-plane modes, rolling upgrade compatibility, legacy import/drain, and bounded B4 removal (ADR-167, Designed). |
