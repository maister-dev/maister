...
C4Component
    ...
    Container_Boundary(supervisor, "Supervisor daemon") {
        ...
        Component(model_catalog, "model-catalog/", "Model resolver (ADR-075)", "ModelSource registry, ACP-probe/provider/curated/CCR sources, in-memory TTL cache, passive harvest.")
    }
...
| `lib/supervisor-client` | `web/lib/supervisor-client.ts` | `createSession`, `sendPrompt`, `deliverPermission`, `cancelPermission`, `deleteSession`, `listSessions`, `checkpointSession`, `streamSession`, `resolveModelSuggestions` (ADR-075). | `lib/errors`, `pino`. |
