...
instance on ONE host. Repository/worktree/Git operations remain local and
shared under ADR-023, so a different host for the supervisor is not supported
in the current target. Stage B makes a separate, narrower cut: for a
`canonical_events_v1` run, Postgres is the durable owner of execution events,
read models, browser replay, prompt results, and runtime-object metadata. The
supervisor retains ACP processes and private runtime files, exposing their
content only through typed opaque-object routes. Runtime event/cost files are
not manager inputs: the private host outbox feeds canonical Postgres state.
The web tier addresses the supervisor as a registered **execution
host** with a durable identity, per-run epoch-fenced **execution assignments**,
an enveloped **command ledger**, and opaque adopted-workspace handles (Implemented —
...
C4Container
    ...
    Rel(supervisor, fs, "Writes private step diagnostics + state.sqlite outbox", "POSIX")
...
| Supervisor daemon   | Implemented | Node 24 + Fastify + pino + Zod                     | Owns ACP sessions, private runtime files, durable host outbox, heartbeat, canonical cost facts, and permission deferreds. Also the local execution host: durable identity + fences + receipts + adopted-workspace handles in private `node:sqlite`.                                                                           |
| Database            | Implemented | Postgres 16                                        | Persistent state for projects, ACP runners, flows, tasks, runs, workspaces, node attempts, HITL, canonical execution events, event consumers, prompt lifecycle, and runtime-object catalog.                                                                                                                                                                                               |
| `claude-agent-acp`  | Implemented | `@agentclientprotocol/claude-agent-acp@0.75.1`     | ACP adapter wrapping Claude Agent SDK. One process per session.                                                                                                                                                                                                                                                                                                                                       |
| `codex-acp`         | Implemented | `@agentclientprotocol/codex-acp@1.10.0`            | ACP adapter bundling Codex. One process per session.                                                                                                                                                                                                                                                                                                                                                  |
...
- **Web ↔ Supervisor** — HTTP + SSE.
  ...
  - [`api/async/supervisor-sse.asyncapi.yaml`](api/async/supervisor-sse.asyncapi.yaml) (SSE event stream).
    Client: `web/lib/execution-host/` (Implemented — ADR-166: `BoundClient` per
...
C4Component
    ...
    Container_Boundary(supervisor, "Supervisor daemon") {
        ...
        Component(outbox, "runtime-event-publisher.ts", "Host outbox publisher", "Commits redacted host event before live publication.")
        ...
        Component(host_state, "host-state.ts + execution-fence.ts + command-receipts.ts + workspace-registry.ts", "Execution-host substrate (Implemented, ADR-166)", "node:sqlite state store: host identity, per-run epoch fences + eviction, command receipts, adopted-workspace handles.")
    }
    ...
    Rel(http_api, host_state, "parseCommandBody() + runCommand(): fence, receipt, resolveForSession()")
    ...
    Rel(outbox, fs, "state.sqlite durable outbox", "node:sqlite")
...
| `runtime-event-publisher` | `supervisor/src/runtime-event-publisher.ts`              | Host event durability.                                 | Commits redacted events to the SQLite outbox before publication.                                                                                                                                          | `host-state`.                                                                |
...
| `host-state`          | `supervisor/src/host-state.ts`                                | Execution-host state store. **Implemented — ADR-167.** | Open `state.sqlite` (WAL) under `MAISTER_EXECUTION_HOST_STATE_DIR`; mint or verify the pinned `hostKey`; persist fences, command receipts, host-outbox sequence/ACK state, and private runtime-object metadata.                                    | `node:sqlite`.                                                            |
| `runtime-objects`    | `supervisor/src/runtime-objects.ts`                           | Host-private object registry.                           | Reserves, atomically seals, ranges, and tombstones bytes under the execution-host state root; public contracts expose only object IDs and verified metadata.                                                                                     | `host-state`, `runtime-event-publisher`.                                  |
| `execution-fence`     | `supervisor/src/execution-fence.ts`                           | Fence enforcement. **Implemented — ADR-166.**          | `hostKey` / `runId` / epoch / `assignmentId` rules in order; persist the high-water BEFORE execution; evict lower-epoch live sessions (`session.exited {reason: fenced}`).                                                                        | `host-state`, `registry`, `pending-permissions`, `types`.                 |
| `command-receipts`    | `supervisor/src/command-receipts.ts`                          | Idempotency receipts. **Implemented — ADR-166.**       | Replay `completed` / `rejected` receipts verbatim (`X-Maister-Command-Replayed`), join in-flight duplicates, `turn_lost` for accepted-without-in-flight (repaired at boot, before any reclamation); serves `GET /commands/:id` and the D6 `POST /commands/:id/retirement` handshake, which compacts a receipt to a tombstone rather than deleting it — receipts are never reclaimed by age.                                                              | `host-state`, `types`.                                                    |
| `workspace-registry`  | `supervisor/src/workspace-registry.ts` + `workspace-roots.ts` | Opaque workspace handles. **Implemented — ADR-166.**   | `POST /workspaces/adopt` validation per kind against `MAISTER_WORKSPACE_ROOTS`; `(runId, realpath)` idempotency; `resolveForSession(handle)` — the ONE path-derivation function feeding spawn, confinement, cost, and the events log.             | `host-state`, `node:fs`, `types`.                                         |
...
C4Component
    ...
    Container_Boundary(web, "Web tier") {
        ...
        Component(supervisor_client, "lib/execution-host/ + lib/supervisor-client.ts", "Execution-host client (Implemented, ADR-166)", "Registrar/resolver, assignment mint, command ledger + deliverer, BoundClient per assignment; supervisor-client.ts is the local-direct transport.")
...
| `lib/supervisor-client`                             | `web/lib/supervisor-client.ts`             | Local-direct wire for create, asynchronous `startPrompt`, prompt receipt, input, cancel, checkpoint, session listing, host outbox replay/ACK, and opaque runtime objects. It is transport-only behind `lib/execution-host/`.                                                                                                          | `lib/errors`, `pino`.                                                                                   |
| `lib/execution-host`                                | `web/lib/execution-host/*`                 | (Implemented — ADR-166) `registrar` (health → `execution_hosts` policy), `resolver` (30 s memo), `assignments` (`mintAssignment` in the claim tx, release), `commands` + `redact` (ledger rows), `ledger` + `deliverer` (per-kind policy table, CAS FSM, retry budgets), `adoption` (lazy `workspace.adopt` from server state), `recovery` (W1/W2/W4 + retention), `placement` (`mintPlacement` inside the claim tx; `ensureAssignment`, the Stage-A lazy mint for never-placed runs), `legacy` (`reportLegacyActiveRuns()`), `client` (`forAssignment` → `BoundClient`, `local` → `HostAdminClient`). | `lib/db`, `lib/errors`, `lib/supervisor-client` (transport only), `pino`.                               |
| `lib/workers`                                       | `web/lib/workers/*`                        | (Implemented — ADR-176) `runtime` — the composition root for the three durable workers, imported ONLY by `instrumentation-node.ts` and deliberately outside `lib/execution-host/` (every domain registry imports that package, so a registry inside it closes a load-time cycle). It composes the five production `PromptOwnerRegistry`s covering all five `PROMPT_OWNER_SHAPES` kinds, asserts that key set (`CONFIG` on disagreement, and on a duplicate kind), and starts/stops the prompt-owner recovery worker plus the flow and agent continuation workers behind three `Symbol.for` process slots. `health` — the read side, with NO domain import, so the `system_sweep` summary can report worker health cycle-free. | `lib/db/client`, `lib/errors`, `lib/server-lifecycle`, the five owner registries, the two continuation workers. |
...
| `lib/flows/runner`                                  | `web/lib/flows/runner.ts`                  | Flow graph execution and resume gate.                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `flows/*`, `db/schema`, `scheduler`, `execution-host` (Implemented — ADR-166; was `supervisor-client`). |
| `app/api/runs`                                      | `web/app/api/runs/route.ts`                | Launch a run from a Backlog task.                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `db`, `worktree`, `scheduler`, `flows/runner`.                                                          |
| `app/api/runs/[runId]/stream`                       | `web/app/api/runs/[runId]/stream/route.ts` | Browser-facing durable run SSE replaying only manager-owned `execution_events` by run sequence.                                                                                                                                                                                                                                                                                                                       | `db`, `execution_events`.                                                               |
| `app/api/runs/[runId]/hitl/[hitlRequestId]/respond` | Route Handler                              | HITL response two-phase claim, permission delivery or atomic artifact write, runner wake-up.                                                                                                                                                                                                                                                                                                                                                                                                   | `db`, `atomic`, `execution-host` (Implemented — ADR-166; was `supervisor-client`), `flows/runner`.      |
...
| `app/api/runs/[runId]/stream/route.ts`                       | Route Handler                                         | SSE bridge replaying only canonical manager events by run sequence.                                                                                                  | Implemented |
...
| Projector                                                    | `web/lib/projector/artifact-projector.ts`             | Web-side. Derives tool-call evidence from ordered canonical `execution_events`; it never drives run state.                                                                            | Implemented |
...
ADR-166): both processes resolve `.maister/` from the same
