...
| Project Brain | `web/lib/brain/*` + `web/lib/domain-events/memory-harvest.ts` | Sub-project A (ADR-122): owned-tier `brain_*` substrate on pgvector — harvest→distill→retain (dedup-or-reinforce), decay sweep, hybrid `RecallRanker` recall (no LLM at read), immutable per-generation embeddings + reindex, embedding-provider registry, `memory_recall`/`memory_retain` MCP tools + P7 ambient, 4-layer enablement. Own migration lineage `web/lib/db/brain-migrations`. See [`system-analytics/project-brain.md`](system-analytics/project-brain.md). | Implemented |
