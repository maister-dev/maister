...
**command ledger**, and opaque adopted-workspace handles (Implemented —
...
| `host-state`          | `supervisor/src/host-state.ts`                                | Execution-host state store. **Implemented — ADR-166.** | Open `state.sqlite` (WAL) under `MAISTER_EXECUTION_HOST_STATE_DIR`; mint or verify the pinned `hostKey` (conflict → fatal); per-process `bootId`; tables `host_identity`, `run_fences`, `workspaces`, `command_receipts`; receipt prune (7 d).    | `node:sqlite`.                                                            |
