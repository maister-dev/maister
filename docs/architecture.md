...
content only through typed opaque-object routes. `run.events.jsonl` and
`cost.jsonl` remain host-local diagnostic/legacy compatibility spools; only an
immutable `legacy_file_v1` run can read them through the web tier before the
B4 drain. The web tier addresses the supervisor as a registered **execution
...
| Supervisor daemon   | Implemented | Node 24 + Fastify + pino + Zod                     | Owns ACP sessions, spawns adapters, heartbeat watcher, cost accounting, permission deferreds, run event log. (Implemented — ADR-166) Also the local execution host: durable identity + fences + receipts + adopted-workspace handles in a private `node:sqlite` state store under `<runtimeRoot>/.maister/execution-host/`.                                                                           |
| Database            | Implemented | Postgres 16                                        | Persistent state for projects, ACP runners, flows, tasks, runs, workspaces, node attempts, and HITL. (Implemented — ADR-166) SSOT for `execution_hosts`, `execution_assignments`, `execution_commands`.                                                                                                                                                                                               |
...
| `cost`                | `supervisor/src/cost.ts`                                      | Cost accounting.                                       | Emits bounded `usage.recorded` facts into the Stage B host outbox. `cost.jsonl` is retained only as a local legacy/diagnostic spool.                                                                                                             | `registry` (channel constant), `runtime-event-publisher`.                 |
| `events-log`          | `supervisor/src/events-log.ts`                                | Legacy event spool.                                    | Writes `run.events.jsonl` only for immutable `legacy_file_v1` runs; it is not a canonical-state source.                                                                                                                                          | `node:fs`.                                                                |
...
| `command-receipts`    | `supervisor/src/command-receipts.ts`                          | Idempotency receipts. **Implemented — ADR-166.**       | Replay `completed` / `rejected` receipts verbatim (`X-Maister-Command-Replayed`), join in-flight duplicates, `turn_lost` for accepted-without-in-flight; serves `GET /commands/:id`.                                                              | `host-state`, `types`.                                                    |
...
| `app/api/runs/[runId]/stream`                       | `web/app/api/runs/[runId]/stream/route.ts` | Browser-facing durable run SSE. Canonical runs replay manager-owned `execution_events` by run sequence; only the explicit legacy branch reads its event spool until B4.                                                                                                                                                                                                                                                                                                                       | `db`, `execution_events`, legacy spool.                                                               |
...
| `app/api/runs/[runId]/stream/route.ts`                       | Route Handler                                         | SSE bridge replaying canonical manager events by run sequence; an immutable legacy-mode branch tails the bounded compatibility spool until B4.                                                                                                  | Implemented |
...
| Projector                                                    | `web/lib/projector/artifact-projector.ts`             | Web-side. Canonical runs derive tool-call evidence from ordered `execution_events`; legacy runs retain the bounded JSONL branch until B4. It never drives run state.                                                                            | Implemented |
