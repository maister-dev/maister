...
4. **Actions** - action shortcuts grouped by risk: conversation/session
   ...
   (ADR-181 — Implemented) Every Actions item is a link. The git actions
...
- Actions use `deriveWorkbenchLifecycleActions` plus promote/delivery policy
  state. (ADR-181 — Implemented) The run detail feeds it the viewer and the open
...
summary uses the same reviewable-path filter and base-to-working-tree source as
the workspace; it therefore includes committed, staged, unstaged, and untracked
reviewable changes. Other scope controls remain forensic and explicitly named.
