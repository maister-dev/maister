...
1. **Run header** - the **task title** as the H1, a `KEY-N` chip beside the run
   ...
   When branch sync and PR lifecycle tracking ship (Implemented, ADR-140/141), the
...
- Branch sync + PR lifecycle (Implemented, ADR-140/141): `POST /api/runs/{runId}/sync`
...
ADR-140/141 add branch-sync and PR-lifecycle labels under the existing `run`
...
Branch sync (ADR-141) and PR lifecycle tracking (ADR-140) add run-finishing
