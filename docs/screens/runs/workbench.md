...
[`run-inspector.md`](run-inspector.md)). Branch sync (ADR-141) adds a **6th**
...
double-fences against promotion (a `sync` claim refuses while promotion is
claiming/done, and `promoteRun` refuses while a `sync` claim is active). The
matrix and claim semantics are canonical in
