...
| `node_attempts.action_resume` | Nullable version-1 JSONB (`0145`) | Orchestrator source command/assignment, admitted current assignment and ordinal, retained ACP resume handle. The normal capacity-checked child-wake claim advances the ordinal and clears the old action snapshot atomically. Recovery consumes this intent without authorizing another turn. |
