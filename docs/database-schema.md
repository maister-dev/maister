...
| `authored_capabilities`       | **(M25 — Implemented, migration `0028`)** Project-local authored rule/skill/flow identity with draft/published pointers and archive state. UNIQUE `(project_id, kind, slug)`. **(ADR-149 — Designed, migration `0118`)** session edit-lock columns (`locked_by_user_id` FK `users` SET NULL, `locked_by_session`, `lock_expires_at`; nullable, no new index; twin of `local_packages`). | `projects.id`, `users.id`                                                  |
...
authored_capabilities {
  ...
  lockedByUserId?,               // (ADR-149 — Designed, migration 0118)
