...
| `run_messages`                         | Run-kind-agnostic transcript ledger (generalized from `scratch_messages`, migration `0085`). Append/upsert message rows with monotonic `sequence`; nullable `node_attempt_id` attributes a flow node session's transcript (NULL for scratch / single-session). Unique `(run_id, node_attempt_id, sequence)` `NULLS NOT DISTINCT` keeps scratch's `(run_id, sequence)` invariant. Nullable `prompt_dispatch_key` (`0170`) identifies a dispatched prompt recorded as a `user` row, under a partial `NULLS NOT DISTINCT` unique index.                                                                                                                                                                                                                                                                                                                                                                                                    | `runs.id`, optional `node_attempts.id`                                                                                                            |
...
| `scheduler_job_runs`                   | **(Implemented, migrations `0027`, `0173`)** Scheduler attempt ledger with status, lease expiry, summary, and error fields. Expired `Claimed`/`Running` attempts are reaped before new claims. Index `(job_id, claimed_at, id)` supports the latest-attempt lookup by backward scan.                                                                                                                                                                                                                                                                                                                                                                                                                              | `scheduler_jobs.id`                                                                                                                               |
...
claiming new work. Migration `0173` adds the ascending physical index
`(job_id, claimed_at, id)`; PostgreSQL scans it backward for the production
`ORDER BY claimed_at DESC, id DESC LIMIT 1` lookup. A running `system_sweep`
renews and fences both durable
...
`prompt_dispatch_key` (migration `0170`) carries the per-dispatch identity of a
RECORDED PROMPT — a `user` row written by the flow dispatcher rather than by the
transcript projector (TRC-05). Its value is derived from the prompt owner's
existing operation-key function, never from a parallel identity scheme.
`run_messages_prompt_dispatch_key_uq` is a PARTIAL unique index on
...
| `execution_commands`        | `execution_commands_host_span_settled_idx`       | `(executionHostId, completedAt)` PARTIAL WHERE `settled_from = 'host_span'`          | **(ADR-167 D5 amendment, Implemented, migration 0177)** Per-host host-span settlement counts on `/admin/execution-host`.                                                           |
...
`0176` lets retirement compaction, and nothing else, drop the request and
...
| --- | --- | --- |
| `execution_commands.request_canonical_json` | `text`, nullable only before activation, for existing non-prompt commands, or on a retired tombstone (`0176`) | Immutable exact JCS UTF-8 request until retirement compaction drops it; `request_schema = maister.command.request.v2`, SHA-256 must match; private, excluded from DTOs/logs. |
...
| `execution_commands.receipt_evidence` | `jsonb`, null | Normalized terminal receipt identity/outcome, independent of event order; conflicting replay quarantines without overwrite. Required while `terminal_evidence_sha256` is set, except on a retired tombstone, which drops it (`0176`). Legacy and strict request-bound v2 storage are implemented. |
| `execution_commands.terminal_event_id` | `text`, null | Exact canonical event ID, resolved against the same command/fence/target; event cannot be pruned while referenced. Required while `terminal_evidence_sha256` is set, except on a `host_span` settlement awaiting canonical confirmation (`0177`, Implemented). |
| `execution_commands.terminal_evidence_sha256` | `text`, null | Lowercase 64-hex digest of the agreed versioned terminal identity. |
| `execution_commands.settled_from` | `text`, null (`0177`, Implemented) | CHECK `execution_commands_settled_from_check`: NULL, or `canonical | host_span` with the digest set. Written only by the evidence reducer, in the UPDATE that first sets `terminal_evidence_sha256`; a confirming re-reduce never writes it, and the re-created `0141` trigger refuses any change once set. NULL means settled before `0177`. |
