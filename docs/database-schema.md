...
| `scheduler_job_runs`                   | **(Implemented, migration `0027`; ordered observation lookup, `0173`)** Scheduler attempt ledger with status, lease expiry, summary, and error fields. Expired `Claimed`/`Running` attempts are reaped before new claims. The `(job_id, claimed_at, id)` index supports deterministic lookup of the prior attempt summary for cross-process sweep observations.                                                                                                                                                                                                                                                                                                                                                                                                                     | `scheduler_jobs.id`                                                                                                                               |
...
claiming new work, and on `(job_id, claimed_at, id)` so a sweep can read the
immediately preceding durable observation with `ORDER BY claimed_at DESC, id
DESC`. A running `system_sweep` renews and fences both durable lease rows; it
persists a successful result only while it still owns the attempt.
`agent_schedules` is indexed on `(project_id, agent_id)` and
