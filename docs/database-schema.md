...
| `execution_commands`        | `execution_commands_host_span_settled_idx`       | `(executionHostId, completedAt)` PARTIAL WHERE `settled_from = 'host_span'`          | **(ADR-167 D5 amendment, Implemented, migration 0176)** Per-host host-span settlement counts on `/admin/execution-host`.                                                           |
...
| `execution_commands.terminal_event_id` | `text`, null | Exact canonical event ID, resolved against the same command/fence/target; event cannot be pruned while referenced. Required while `terminal_evidence_sha256` is set, except on a `host_span` settlement awaiting canonical confirmation (`0176`, Implemented). |
| `execution_commands.terminal_evidence_sha256` | `text`, null | Lowercase 64-hex digest of the agreed versioned terminal identity. |
| `execution_commands.settled_from` | `text`, null (`0176`, Implemented) | CHECK `execution_commands_settled_from_check`: NULL, or `canonical | host_span` with the digest set. Written only by the evidence reducer, in the UPDATE that first sets `terminal_evidence_sha256`; a confirming re-reduce never writes it, and the re-created `0141` trigger refuses any change once set. NULL means settled before `0176`. |
