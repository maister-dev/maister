...
| `node_attempts.action_resume` | Nullable version-1 JSONB (`0145`, expanded by `0147`) | Source command/assignment, admitted current assignment and ordinal, retained ACP resume handle. `kind: orchestrator` is authorized by the capacity-checked child-wake claim; `kind: permission` additionally binds the original HITL request, request ID and selected option in the capacity-checked idle-resume claim. Both advance the ordinal and clear the old action snapshot atomically. Recovery consumes this intent without authorizing another turn. |
