...
- 2026-09-02 — **D-3 narrowed by ADR-163, not reversed.** The grace anchor stays
  the `runs.review_entered_at` column and a TOP-LEVEL flow `Review` still emits
  no domain event. Delegated children (`parent_run_id` set) DO emit `run.review`
  on every Review flip through one helper (`emitDelegatedReviewIfChild`), because
  their parked orchestrator waits on it; the `agentTriggers` population widening
  D-3 warned about is therefore limited to delegated children, by design.
