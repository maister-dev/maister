...
**Amendment (2026-09-18, [ADR-175](adr-175.md)): the crash-resume mode above
described only the `redispatch` arm.** The `resume-agent` arm shipped a
different path — `createSession({resumeSessionId})` handed to the NeedsInput
permission driver — which stopped recovering anything once owned prompts became
generation-fenced, because the crashed attempt stays bound to the retired
assignment epoch and `admitNodePrompt` refuses its prompt as
`node_admission_generation`. ADR-175 routes the agent arm through this same
`crashResume` door, so the sentence above is now true of BOTH arms. The decision
recorded here is unchanged; only its coverage was incomplete.

**Amendment (2026-09-11): recover and discard reach the external surface.**
The RBAC clause above constrains the *permission* — a `recoverRun` project
action at min role `member`, deliberately decoupled from `launchRun` — not the
*credential*. It was read as the latter, so both operations shipped only as
internal routes behind `requireActiveSession`, and a run driven entirely over
`/api/v1/ext/*` with a project token stalled on the first lost adapter: every
other lifecycle verb (launch, promote, sync, reopen, rework, cancel, collect,
delegate, message, plan, HITL respond) was reachable, recovery was not.

`POST /api/v1/ext/runs/{runId}/recover` and
`POST /api/v1/ext/runs/{runId}/discard` close that, under ONE new token scope
`runs:recover` mapped to the ONE `recoverRun` action this ADR defined for the
pair. Discard ships with recover because this ADR's own refusals name it: a
`discard-only` or `unresumable` classification tells the caller to discard.
Discard is how an unattended caller ENDS an attempt it cannot recover — flipping
the run `Abandoned`, archiving the branch, releasing the worktree, and emitting
`run.abandoned` — rather than leaving a `Crashed` row with a live worktree until
the 7-day GC sweep. It is not a precondition for relaunching: `Crashed` is
already `launchable` (`RUN_STATUS_LAUNCHABILITY`).

Nothing about the recovery mechanism changes. The properties §3.2 already
specified are exactly what makes an automated caller safe: the Phase-1 CAS on
`status='Crashed'` answers a concurrent second call `409` with no double-spawn,
cap re-admission keeps `MAISTER_MAX_CONCURRENT_RUNS` intact, and
`discard-only`/`unresumable` are terminal (`409`/`410`), not retryable — a bot in
a retry loop collects refusals. Only `503` is worth retrying.

The "explicit human Recover" language in
[`system-analytics/reconciliation-gc.md`](../system-analytics/reconciliation-gc.md)
is an anti-*implicit* rule and is unaffected: the reconciler still must not
silently auto-resume a mid-turn agent or re-run a half-executed `cli`. A caller
issuing this route has made that decision explicitly; only the credential
carrying it differs.

*Correction to the Discard clause above, noted while wiring the external route.*
The body of this ADR says Discard is "NOT a synchronous worktree delete" and
"never calls `preserveWorktree`/`removeOwnedWorktree`". That was superseded by
the M27 workbench lifecycle and is stale: `POST /api/runs/{runId}/discard` now
resolves to `discardWorkbench → removeWorkbenchForCtx`, which claims the
lifecycle operation, preserves the branch (ADR-035), removes the worktree, and
records the drop with `nextRunStatus = 'Abandoned'`. The external route is the
same path, so it inherits that behavior — no work is lost, but the worktree is
gone when the call returns. See
[`system-analytics/workbench-lifecycle.md`](../system-analytics/workbench-lifecycle.md).

`runs:recover` is deliberately absent from `AGENT_TOKEN_SCOPES`,
`ORCHESTRATOR_TOKEN_SCOPES`, and `CROSS_PROJECT_AGENT_SCOPES` — as every other
`runs:*` scope is. The `RecoverResult → HTTP` projection moved to
`web/lib/runs/recover-http.ts` so the two surfaces cannot drift; that divergence
was already latent, since the generic ext code mapper has no `CHECKPOINT` case
and would have answered `unresumable` as `500` rather than `410`.
