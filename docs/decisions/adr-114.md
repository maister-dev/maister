...
> **Resolution amendment (2026-09-14):** The binding requirement and
> default-only chain in items 3 and 5 are superseded for Flow sessions by the
> current [session resolution contract](../system-analytics/sessions.md#expectations).
> Configured launch/task/Flow/project/platform choices resolve compatible unbound
> sessions before intent matching. Explicit slot bindings remain authoritative;
> consensus slot resolution is unchanged.
...
- **2026-09-25** — `run_session_incarnations.steering_supported` records, per
  incarnation, whether the adapter advertised `_meta.steering.supported` on its
  `initialize` (NULL = not observed: an incarnation created before migration
  `0180` or by a host older than that contract). It is written once by
  `applyCreateAck` and never per logical session, because every resume spawns a
  fresh adapter process that initializes again. (Implemented —
