...
- **2026-09-21 — [ADR-179](adr-179.md).** The Transport paragraph's "Secrets
  accepted only as `env:NAME`" no longer holds. MCP env and header entries are
  `Record<name, value>` maps whose values are `literal | env:NAME` under one
  shared grammar, plus a dedicated `bearerTokenEnv` field; a LITERAL is the
  operator's declaration that the value is not a secret and the form warns
  inline under secret-shaped keys instead of refusing. The invariant this
  sentence was protecting survives verbatim for REFERENCES: the value behind an
  `env:NAME` is resolved only on the execution host and is never stored, logged,
  or echoed in any response. `env_keys` / `header_keys` are dropped by migration
  `0172_mcp_env_values`.
- **2026-09-21 — [ADR-179](adr-179.md).** The "Required vs additional MCP at
  launch" rule gains a second `EXECUTOR_UNAVAILABLE` reason beside
  `supported_agents`: a REQUIRED ref whose transport the launch adapter cannot
  use (codex + `sse`) refuses at the same precondition, and an ADDITIONAL one is
  withheld with the persisted reason `agent-unsupported-transport`.
