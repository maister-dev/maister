...
- **Taxonomy = 12 curated types, never raw `session/update`.** `run.started`,
  ...
  `run.pr_merged` / `run.pr_closed` / `run.pr_conflicts`, ADR-161's
  `run.rework_claimed` / `run.rework_returned`, and ADR-172's four `attention.*`
  types — the last of these user-scoped rather than run-scoped.
