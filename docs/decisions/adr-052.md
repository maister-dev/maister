...
- **2026-09-17** — The Context paragraph's description of the run SSE stream as
  carrying `session.line|update|permission_request|exited|crashed` has been
  stale since ADR-167: the browser stream is served from manager-owned
  `execution_events`, not from a supervisor session feed, and a route handler
  never tails a host file. The decision itself is unaffected — this ADR is about
  what the stream does NOT carry (a `nodeId→status` delta), which remains true.
  Recorded while aligning the transcript and artifact contracts for the
  run-trace plane; see
  [`system-analytics/run-trace.md`](../system-analytics/run-trace.md).
