...
> **2026-09-02:** [ADR-164](adr-164.md) removes the CCR model source and
> router fields. ACP-probe, provider, and curated discovery plus configured-model
...
**Amendment (2026-09-09).** With `@agentclientprotocol/sdk` 1.4.0 (and
`codex-acp` 1.10.0 / `claude-agent-acp` 0.75.1) the pre-1.0
`unstable_setSessionModel` call and the `models` (`SessionModelState`) response
field left the protocol. The supervisor now reads the "model" select of
`configOptions` on the `session/new` and `session/resume` responses
(`readSessionModels`, `supervisor/src/session-models.ts`) and pins codex via
`session/set_config_option` on that option; an adapter that exposes no "model"
option degrades to the same advisory as a failed call. `models` is still read as
a legacy fallback observation because codex-acp keeps sending it as an untyped
extension (ids suffixed `[<effort>]`). The advisory-never-fails rule and the
claude verify-only channel are unchanged; ADR-084's "only when the adapter
supports it" gate now means "exposes a model config option".
