...
scheduler timer) and periodically inside the seeded `system_sweep.default`
job. Its core is a pure
...
- `MAISTER_RECONCILE_GRACE_SECONDS` configures classification grace; no new
  port, sidecar, or wire change.

**2026-09-22 amendment (P0-6):** the independent reconcile interval was
retired. `MAISTER_RECONCILE_SWEEP_INTERVAL_SECONDS` is ignored with a boot WARN;
periodic reconcile runs inside the scheduler-owned `system_sweep` cadence.
