...
> **Amended by [ADR-180](../decisions.md#adr-180-permission-deadline-has-one-owner) (2026-09-22):** the keep-alive window below is owned by the **web alone** — `MAISTER_KEEPALIVE_MINUTES` is a web variable and the supervisor no longer reads it. The host keeps an absolute `MAISTER_PERMISSION_MAX_HOURS` cap (default 24) whose expiry performs the same graceful checkpoint as phase 2, never a reject.
