...
- **2026-09-21 — [ADR-179](adr-179.md).** The per-project overlay (W-C) no
  longer rewrites NAMES. `config_overlay` keeps its wire shape and its
  write-time slot validation, but application replaces the VALUE for a declared
  key and PRESERVES the key — renaming the child variable meant the server never
  received the variable it reads. Overlay values use the same
  `literal | env:NAME` grammar as server values, and `bearerTokenEnv` is
  overridable for http/sse targets. Slots are the keys of the target's
  `env`/`headers` maps, which also gives package-TEMPLATE targets the slots
  `resolveBindTarget` could not previously name. Stored overlay rows stay valid
  and are not rewritten by migration `0172_mcp_env_values`.
- **2026-09-21 — [ADR-179](adr-179.md).** The probe (W-F) carries maps rather
  than name lists; references travel the wire unresolved and are resolved only
  on the host. Health is joined by host-scoped env-ref PRESENCE:
  `POST /diagnostics/env-refs` answers name-by-name without returning values,
  and the verdict is cached at write time on all three row kinds (platform
  `readiness_status`/`readiness_reasons`, project and package
  `material.readiness`) with the reasons rendered in the panels.
- **2026-09-21 — [ADR-179](adr-179.md).** The withheld-reason union gains
  `agent-unsupported-transport`, persisted into `runs.withheld_mcps` and
  `node_attempts.materialization_plan` exactly like the two trust reasons, after
  them in pass order. `gateAndOverlayMcpServers` becomes true to its own header:
  scratch launches now take the same gate as flow and agent launches.
