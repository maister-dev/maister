...
**Amendment (2026-09-09).** Native defaults are bootstrapped only into an
EMPTY catalog. The original upsert-if-absent on every `/settings` load
resurrected defaults an admin had deliberately deleted (a five-adapter host
re-created `gemini-cli`/`opencode-native`/`mimo-code-native` after each
delete) — a populated catalog now changes only through explicit admin actions
(the preset "create" buttons). The absent-singleton default is chosen among
ALL enabled `Ready` runners in adapter preference order (native default first,
then the adapter's other runners by id), so an admin-created runner becomes
the platform default instead of being ignored. Readiness recompute (single
writer) is unchanged.
