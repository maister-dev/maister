...
7. **`hook_trip` is a dedicated HITL kind** (and `assignments.action_kind`), added by **migration 0063** — the one schema change. Follows the `budget_breach` precedent (one kind per escalation cause; `budget_breach` is already in both enums).
