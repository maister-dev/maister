...
erDiagram
    ...
    WORKSPACES {
        ...
        text lifecycle_operation_name "M27 0032 archive|drop|exportBranch|snapshotCommit|handoffBranch|sync (sync: ADR-140 sync claim, app-level, no CHECK)"
    }
...
- **(ADR-139, migration 0103, Implemented)** `workspaces_pr_state_scan_idx` partial index
  ...
  project).
- **(ADR-140, migration 0104, Implemented)** `run_sync_attempts_run_attempt_uq` on
