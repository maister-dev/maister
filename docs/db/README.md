...
| [`execution-hosts-domain.md`](execution-hosts-domain.md) | Execution-host tables: `execution_hosts`, `execution_assignments`, `execution_commands` + the attribution columns on `runs` / `run_sessions` / `node_attempts` (ADR-166, migration `0130`, Implemented). |
