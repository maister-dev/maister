...
| `MAISTER_GC_SWEEP_INTERVAL_SECONDS` | no | `3600` | **Deprecated by ADR-140 (Designed).** The legacy in-process GC timer interval; the target runtime routes periodic cleanup through the `system_sweep` scheduler job. |
