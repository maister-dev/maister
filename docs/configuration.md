...
| `MAISTER_EXECUTION_HOST_STATE_DIR` | no | `<MAISTER_RUNTIME_ROOT>/.maister/execution-host/` | **(Implemented — [ADR-164](decisions.md#adr-164-local-execution-host-contract--durable-host-identity-epoch-fenced-assignments-command-ledger-opaque-adopted-workspaces).)** Supervisor only. Directory of the supervisor-private `node:sqlite` execution-host state store (`state.sqlite`: host identity, per-run epoch fences, adopted-workspace handles, command receipts). Runtime-root-relative by default so per-lane runtime roots never collide; inside the gitignored `.maister/`. Unwritable → fatal boot error `execution-host-state-unwritable`. The web tier never reads it. Host/service-env only ([ADR-023](decisions.md#adr-023-run-web--supervisor-on-the-host-containerize-only-postgres)). |
| `MAISTER_EXECUTION_HOST_KEY` | no | unset | **(Implemented — ADR-164.)** Supervisor only. Optional execution-host identity pin (`^[A-Za-z0-9_-]{8,64}$`). Applied when no key is stored (first boot, restored state dir, migration to a new machine) or when it equals the stored key. A pin that CONFLICTS with the stored key **refuses boot** (fatal `execution-host-key-conflict`, exit 1 — unset the pin or deliberately wipe the state dir). Never logged beyond a prefix. Host/service-env only. |
| `MAISTER_WORKSPACE_ROOTS` | no | `~/.maister/worktrees:~/.maister/local:<MAISTER_RUNTIME_ROOT>/.maister` | **(Implemented — ADR-164.)** Supervisor only. Colon-separated absolute directories (`~` expanded, realpath'd at boot, WARN on a missing root) that a `git_worktree` or `directory` workspace adoption (`POST /workspaces/adopt`) must resolve under; a `repo_checkout` adoption is exempt (arbitrary repo location, ADR-023). **A moved web `MAISTER_WORKTREES_ROOT` / `MAISTER_LOCAL_PACKAGES_ROOT` MUST be mirrored here** or every launch fails at adoption with `PRECONDITION {reason: workspace_rejected, rule: outside_roots}` (the rejection log names the remediation). Host/service-env only; never a compose var. |
