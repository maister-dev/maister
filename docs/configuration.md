...
| `MAISTER_EVENT_STREAM_STALL_SECONDS`           | no                                                                   | `300`                                                             | **(Implemented.)** Web: execution event-stream stall window (seconds). There is no heartbeat event type and one `execution_event_streams` row serves a whole host, so a quiet stand is normal — a stream counts as stalled only when an OPEN command (`delivering`/`accepted`) produced no event for this long while `execution_hosts.readiness = 'ready'`. The `system_sweep` pass then restarts the stream's consumer loop and records the attempt; only a stream a restarted consumer still cannot advance is degraded to `state='lost'`. Every committed ingest batch advances `last_seen_at`, and a batch is cut at most 250 ms after its first row, so batching never reads as silence (Designed — ADR-167 amendment 2026-09-25). Floor 1. Read by `web/lib/instance-config.ts:eventStreamStallSeconds()`; enforced by `web/lib/execution-host/events/stream-health.ts`. Host/service-env only ([ADR-023](decisions.md#adr-023-run-web--supervisor-on-the-host-containerize-only-postgres)). |
...
| Event ingest (manager, Designed 2026-09-25) | Batch of `N` rows (`MAISTER_EVENT_INGEST_BATCH_ROWS`, default 200) or 4 MiB payload or 250 ms after its first row; reader buffer `4 × N` rows / 16 MiB; contiguity walk reads one 500-sequence window (≤ 500 rows) per read | One transaction and at most one ACK per batch; 250 ms stays under half the 30 s claim lease, which each batch renews. A full buffer stops reading the socket so the host pauses the subscriber instead of the manager growing memory. See [batched ingest](system-analytics/execution-event-plane.md#batched-ingest-and-a-pausing-subscriber). |
