...
| `MAISTER_SCHEDULER_TIMER_ENABLED`              | no                                                                   | auto                                                               | **P0-6.** Unset starts the fallback timer unless `MAISTER_CRON_TOKEN` selects external ownership. Literal `true` always starts fallback; literal `false` requires a token or resolves `missing_tick` with a boot WARN. Empty is unset; any other value is `CONFIG`. Single-box deployment sets `true` explicitly. |
...
| `MAISTER_EVENT_STREAM_LAG_SECONDS`             | no                                                                   | `120`                                                             | **P0-7.** Positive safe-integer seconds an ingest/projection backlog must remain above 100 before it can qualify for the three-progressing-sweep lag WARN. Invalid/empty values refuse web boot with `CONFIG`. Observability only: it never changes stall/lost state. |
