...
> | `MAISTER_VAPID_PUBLIC_KEY` | no | unset | **(Implemented, ADR-172.)** VAPID public key for web push. The browser needs it to call `pushManager.subscribe`, so it is the one piece of VAPID material that reaches the client. Web tier only — host/service-env, never a `compose.yml` var (see [ADR-023](decisions.md#adr-023-run-web--supervisor-on-the-host-containerize-only-postgres)). |
> | `MAISTER_VAPID_PRIVATE_KEY` | no | unset | **(Implemented, ADR-172.)** VAPID private key, read server-side by the push sender only. Never logged, never returned by any route. Web tier only — host/service-env, never a `compose.yml` var. |
> | `MAISTER_VAPID_SUBJECT` | no | unset | **(Implemented, ADR-172.)** A `mailto:` or `https:` contact the push service can reach about this deployment's deliveries. Web tier only — host/service-env, never a `compose.yml` var. |
