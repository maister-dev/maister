...
| `MAISTER_EVENT_STREAM_STALL_SECONDS`           | no                                                                   | `300`                                                             | **(Implemented.)** Web: execution event-stream stall window (seconds). There is no heartbeat event type and one `execution_event_streams` row serves a whole host, so a quiet stand is normal — a stream counts as stalled only when an OPEN command (`delivering`/`accepted`) produced no event for this long while `execution_hosts.readiness = 'ready'`. The `system_sweep` pass then restarts the stream's consumer loop and records the attempt; only a stream a restarted consumer still cannot advance is degraded to `state='lost'`. Floor 1. Read by `web/lib/instance-config.ts:eventStreamStallSeconds()`; enforced by `web/lib/execution-host/events/stream-health.ts`. Host/service-env only ([ADR-023](decisions.md#adr-023-run-web--supervisor-on-the-host-containerize-only-postgres)). |
