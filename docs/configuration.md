...
| Event ingest (manager, Designed 2026-09-25) | Batch of `N` rows (`MAISTER_EVENT_INGEST_BATCH_ROWS`, default 200) or 4 MiB payload or 250 ms after its first row; reader buffer `4 × N` rows / 16 MiB; contiguity walk reads 500 pending rows per page | One transaction and at most one ACK per batch; 250 ms stays under half the 30 s claim lease, which each batch renews. A full buffer stops reading the socket so the host pauses the subscriber instead of the manager growing memory. See [batched ingest](system-analytics/execution-event-plane.md#batched-ingest-and-a-pausing-subscriber). |
