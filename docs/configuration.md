...
project:
  ...
  public_branch_template: feature/{task_key}-{slug} # ADR-181; the default
...
| `project.public_branch_template` | `feature/{task_key}-{slug}` | **(ADR-181.)** Template for the PUBLIC name a run branch is published under; the internal branch stays the run's identity and is never renamed. Placeholders: `{task_key}` (`<projects.task_key>-<tasks.number>`, or `run-<first 8 hex of the run id>` for a task-less run), `{slug}` (the task title transliterated Cyrillic → Latin by a fixed table, lower-cased, `[a-z0-9-]`, at most 40 characters; when empty its separator collapses), `{attempt}` (the attempt number; `1` for agent and scratch runs). Validated at parse and at registration: known placeholders only, `{task_key}` required (the one placeholder unique per task, so two tasks — or every task-less run — never render one name), and a valid branch name both with a task and without one. A template that renders an invalid branch name refuses the publish with `CONFIG` (`details.reason: public_branch_template_invalid`) and the dialog falls back to an editable name. Stored in `projects.public_branch_template` at registration and shown read-only in project settings; like `branch_prefix`, a later manifest edit is not re-synced into an already-registered project. |
...
| `MAISTER_UPGRADE_MAINTENANCE`                  | no                                                                   | off                                                               | **Implemented (S4.1).** The staged execution data-plane upgrade fence. Accepts `1`/`true`/`on`/`yes`. While it is set, the web tier admits no run (launch, queue promotion, scratch or package-assistant turn), starts no new agent turn on the prompt ledger, claims no scheduler job and runs no system sweep; every refusal carries `PRECONDITION` with `details.reason="upgrade_maintenance_fence"`. Draining controls (cancel, checkpoint, HITL delivery, promotion of finished work) stay open. Set it on every web process before freezing legacy sources, and unset it after the cut-over. See [execution data cutover](system-analytics/execution-data-cutover.md). |
| `MAISTER_LEGACY_RUNTIME_ROOT`                  | only for `execution-data-plane:import-legacy`                        | —                                                                 | **Implemented (S4.2–S4.6).** The frozen Stage A runtime root (`<root>/.maister/<slug>/runs/<runId>/`) the import CLI reads during the cut-over — a local operator input confined to the inventory, copy, rows and verify phases. Never set on the web tier: after the cut-over the web process must not be able to reach it (D9 step 11). |
| `MAISTER_IMPORT_ADMISSION_DIR`                 | only for the supervisor in import mode; read by the import CLI       | —                                                                 | **Implemented (S4.3–S4.6).** The operator-owned, durable manifest directory (`0700`): the frozen manifest `import-<id>.sqlite`, the supervisor's progress ledger and the maintenance Unix socket `admission/import.sock` live there. Setting it together with `MAISTER_IMPORT_ADMISSION_ID` boots the supervisor with the maintenance import listener; the CLI takes `--manifest-dir` from it when the flag is absent. Keep it outside the web process's OS authority and remove it with the import authority. |
| `MAISTER_IMPORT_ADMISSION_ID`                  | only with `MAISTER_IMPORT_ADMISSION_DIR`                             | —                                                                 | **Implemented (S4.3).** The single import id the supervisor admits (`^[A-Za-z0-9._:-]{1,64}$`), one for the whole cut-over. Each boot mints the next generation and logs it as `import_admission_enabled`; the CLI must carry that generation, and a stale one refuses. |
...
| `MAISTER_EVENT_STREAM_STALL_SECONDS`           | no                                                                   | `300`                                                             | **(Implemented.)** Web: execution event-stream stall window (seconds). There is no heartbeat event type and one `execution_event_streams` row serves a whole host, so a quiet stand is normal — a stream counts as stalled only when an OPEN command (`delivering`/`accepted`) produced no event for this long while `execution_hosts.readiness = 'ready'`. The `system_sweep` pass then restarts the stream's consumer loop and records the attempt; only a stream a restarted consumer still cannot advance is degraded to `state='lost'`. Every committed ingest batch advances `last_seen_at`, and a batch is cut at most 100 ms after its first row, so batching never reads as silence (Implemented — ADR-167 amendment 2026-09-25). Floor 1. Read by `web/lib/instance-config.ts:eventStreamStallSeconds()`; enforced by `web/lib/execution-host/events/stream-health.ts`. Host/service-env only ([ADR-023](decisions.md#adr-023-run-web--supervisor-on-the-host-containerize-only-postgres)). |
| `MAISTER_RECONCILE_SWEEP_INTERVAL_SECONDS`     | retired                                                              | ignored                                                           | **Retired P0-6.** Presence logs one boot WARN and never refuses boot. Reconciliation runs inside `system_sweep.default` on the unified scheduler clock every 60 seconds. See [deployment](deployment.md#scheduler-clock). |
...
| `MAISTER_GC_AGE_DAYS`                          | no                                                                   | `14`                                                              | Web: age before Abandoned/Done/Failed worktrees (Failed: ADR-181) + Removed flow revisions are GC'd                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `MAISTER_GC_ARCHIVE_PUSH`                      | no                                                                   | `false`                                                           | Web: push the `maister/archive/<runId>` branch to the remote during GC preserve **and** (ADR-181) operator archive/drop                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `MAISTER_CRON_TOKEN`                           | no (empty ⇒ `/api/cron/gc` and `/api/cron/tick` return 503 disabled) | (none)                                                            | **Server-only secret** for token-guarded cron routes — never logged or streamed. An unset timer plus a nonempty token resolves to `external_tick`; configuration is not proof of invocation. `/api/cron/gc` remains a compatibility wrapper. See [deployment](deployment.md#scheduler-clock). |
| `MAISTER_SCHEDULER_TIMER_ENABLED`              | no                                                                   | auto                                                               | **Implemented (P0-6).** Unset starts the fallback timer unless `MAISTER_CRON_TOKEN` selects external ownership. Literal `true` always starts fallback; literal `false` requires a token or resolves `missing_tick` with a boot WARN. Empty is unset; any other value is `CONFIG`. Single-box deployment sets `true` explicitly. |
...
| `MAISTER_EVENT_STREAM_LAG_SECONDS`             | no                                                                   | `120`                                                             | **Implemented (P0-7).** Positive safe-integer seconds an ingest/projection backlog must remain above 100 before it can qualify for the three-progressing-sweep lag WARN. Invalid/empty values refuse web boot with `CONFIG`. Observability only: it never changes stall/lost state. |
...
| `MAISTER_PERMISSION_MAX_HOURS`                 | no                                                                   | `24`                                                              | **(Implemented — ADR-180.)** Supervisor-only. Absolute cap on ONE pending ACP permission; on expiry the host runs the graceful checkpoint teardown (cancel the deferreds, SIGTERM, wait the kill grace), never a reject and never a SIGKILL. Parsed as a positive **float**, so FRACTIONAL values are accepted — the integration lane drives sub-second caps (`0.0005` ≈ 1.8 s); an unparseable or non-positive value falls back to `24` with one WARN. This is NOT the keep-alive window: the web owns that. |
...
> | `MAISTER_KEEPALIVE_SWEEP_INTERVAL_SECONDS` | retired | ignored | **Retired P0-6.** Presence logs one boot WARN and never refuses boot. Keepalive passes run inside `system_sweep.default` on the unified scheduler clock every 60 seconds. |
...
> | `MAISTER_VAPID_PUBLIC_KEY` | no | unset | **(Implemented, ADR-173.)** VAPID public key for web push. The browser needs it to call `pushManager.subscribe`, so it is the one piece of VAPID material that reaches the client. Web tier only — host/service-env, never a `compose.yml` var (see [ADR-023](decisions.md#adr-023-run-web--supervisor-on-the-host-containerize-only-postgres)). |
> | `MAISTER_VAPID_PRIVATE_KEY` | no | unset | **(Implemented, ADR-173.)** VAPID private key, read server-side by the push sender only. Never logged, never returned by any route. Web tier only — host/service-env, never a `compose.yml` var. |
> | `MAISTER_VAPID_SUBJECT` | no | unset | **(Implemented, ADR-173.)** A `mailto:` or `https:` contact the push service can reach about this deployment's deliveries. Web tier only — host/service-env, never a `compose.yml` var. |
> | | | | **All three are optional together (`NTF-10`).** With any of them unset the web tier boots, logs `push unavailable — unset: <names>`, and refuses `POST /api/push/subscribe` with `CONFIG`; nothing else degrades. Generate a pair with `npx web-push generate-vapid-keys`. |
> | `MAISTER_EXECUTION_HOST_STATE_DIR` | no | `<MAISTER_RUNTIME_ROOT>/.maister/execution-host/` | **(Implemented — [ADR-166](decisions.md#adr-166-local-execution-host-contract--durable-host-identity-epoch-fenced-assignments-command-ledger-opaque-adopted-workspaces).)** Supervisor only. Directory of the supervisor-private `node:sqlite` execution-host state store (`state.sqlite`: host identity, per-run epoch fences, adopted-workspace handles, command receipts). Runtime-root-relative by default so per-lane runtime roots never collide; inside the gitignored `.maister/`. Unwritable → fatal boot error `execution-host-state-unwritable`. The web tier never reads it. Host/service-env only ([ADR-023](decisions.md#adr-023-run-web--supervisor-on-the-host-containerize-only-postgres)). |
> | `MAISTER_EXECUTION_HOST_KEY` | no | unset | **(Implemented — ADR-166.)** Supervisor only. Optional execution-host identity pin (`^[A-Za-z0-9_-]{8,64}$`). Applied when no key is stored (first boot, restored state dir, migration to a new machine) or when it equals the stored key. A pin that CONFLICTS with the stored key **refuses boot** (fatal `execution-host-key-conflict`, exit 1 — unset the pin or deliberately wipe the state dir). The key is an identity, not a secret: it is logged in full at boot (`execution-host-identity`) and at registration (`execution-host-registered`) and returned by `GET /health`; only the `execution-host-key-conflict` error prints prefixes. Host/service-env only. |
> | `MAISTER_WORKSPACE_ROOTS` | no | `~/.maister/worktrees:~/.maister/local:<MAISTER_RUNTIME_ROOT>/.maister` | **(Implemented — ADR-166.)** Supervisor only. Colon-separated absolute directories (`~` expanded, realpath'd at boot, WARN on a missing root) that a `git_worktree` or `directory` workspace adoption (`POST /workspaces/adopt`) must resolve under; a `repo_checkout` adoption is exempt (arbitrary repo location, ADR-023). **A moved web `MAISTER_WORKTREES_ROOT` / `MAISTER_LOCAL_PACKAGES_ROOT` MUST be mirrored here** or every launch fails at adoption with `PRECONDITION {reason: workspace_rejected, rule: outside_roots}` (the rejection log names the remediation). Host/service-env only; never a compose var. |
...
| Event ingest (manager, Implemented 2026-09-26) | Batch of `N` rows (`MAISTER_EVENT_INGEST_BATCH_ROWS`, default 200) or 4 MiB payload or 100 ms after its first row; reader buffer `4 × N` rows / 16 MiB; contiguity walk reads one 500-sequence window (≤ 500 rows) per read | One transaction and at most one ACK per batch; 100 ms (chosen by the R20 sweep) stays under half the 30 s claim lease, which each batch renews. A full buffer stops reading the socket so the host pauses the subscriber instead of the manager growing memory. See [batched ingest](system-analytics/execution-event-plane.md#batched-ingest-and-a-pausing-subscriber). |
...
| `MAISTER_EVENT_INGEST_BATCH_ROWS` | web | `200` | (Implemented — [ADR-167](decisions.md#adr-167-durable-execution-host-event-and-runtime-data-plane) amendment 2026-09-25; the default is the smallest `N` whose R20 lag stayed at or below `N`.) Runtime events committed per ingest transaction and acknowledged per ACK; range 1–1000, a canonical positive integer. An invalid or empty value refuses web boot with `CONFIG`. |
