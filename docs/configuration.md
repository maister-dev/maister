...
| `MAISTER_MAX_OPERATOR_RESTARTS` | no | `10` | **(ADR-161 — Implemented.)** Web tier. Per-run hard cap on OPERATOR-initiated node restarts (`node_attempts.decision='operator_interrupt'`). These are deliberately EXCLUDED from a flow's `rework.maxLoops` budget — that budget expresses the flow author's tolerance for *automated* rework loops, and charging human intervention to it would let a reviewer exhaust a flow's allowance by helping it — so the bound is not removed, only moved here. Reaching the cap disables `restart_node` / `restart_from` in the server-owned option matrix and refuses a submitted restart with `CONFLICT`; `resume` and `stop` are never capped. Host env only (compose containerizes Postgres alone). |
