...
> | `MAISTER_KEEPALIVE_SWEEP_INTERVAL_SECONDS` | no | `30` | Interval of `startKeepaliveSweeper`'s `globalThis` timer in `web/lib/runs/keepalive-sweeper.ts`, which has **no production caller**. The live driver of `runSweepTick()` is the `system_sweep` job on the scheduler clock (60 s tick) — this variable does not change that cadence. |
