...
## Публичные имена веток

`project.public_branch_template` задаёт имя публикации ветки прогона.
По умолчанию это `feature/{task_key}-{slug}`. Обязателен `{task_key}`;
`{slug}` и `{attempt}` необязательны. Без задачи ключ строится из id прогона.
Результат должен быть допустимым именем Git-ветки. Существующий upstream или
записанная публикация имеют приоритет. См. [Git-действия](/ru/guides/run-git).
