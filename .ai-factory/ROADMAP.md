...
- [x] **M14. Scoped capability materialization** — make node and session
...
- [x] **M20. Dogfood + external adoption signal** — dogfood on MAIster itself
  and installations by other users are confirmed (owner validation,
  2026-07-15). This confirms use beyond an isolated local demo; repository
  state does not reconstruct the original installation count or shipped-change
  metrics. M45 owns the next measurable gate: repeatable core-package process
  runs on internal/private projects.
...
- [x] **ADR-148. Run workspace lifecycle cleanup and reconciliation** — shipped 2026-07-16. Generated migrations `0107` and `0108` add renewable/fenced lifecycle and scheduler leases, removal results, reconciliation observations, and database checks for non-negative attempts plus resolved-state/timestamp pairing. Archive, drop, and discard share the preserve-first removal coordinator; only `Done`/`Abandoned` are automatic TTL candidates, while `Review`/`Crashed`/`Failed` remain until explicit user action. The sole `system_sweep` owner renews its attempt lease, persists its summary, fails for service-level bundle failures, and isolates candidate retries/quarantine. Run JSONL/artifact retention is deliberately deferred.

- [x] **M24. Scheduler service (one clock, polymorphic jobs)** — shipped 2026-06-05 via `feature/m24-m25-wave1-long-lead`. Adds ADR-060, the spec SSOT, migration `0027_m24_scheduler_service.sql`, `scheduler_jobs`, `scheduler_job_runs`, and `agent_schedules`; token-guarded `GET/POST /api/cron/tick`; `/api/cron/gc` compatibility wrapper over shared `system_sweep`; fixed-interval catch-up-not-backfill claim core; stuck-attempt reaping; command/agent/flow handler seams; scheduler status read model; and docs/OpenAPI/env updates. **P0-6/P0-7 amendment (2026-09-22):** single-box installs now default to the fallback timer unless explicitly disabled or external cron is configured; the standalone clock card, overlap telemetry, supervisor stream health, bounded manager lag read model, and admin execution-host diagnostics make recovery-clock and event-plane delay visible without changing stall/lost semantics. The supervisor remains DB-free, and `flow_run` is the only scheduler kind that reaches the existing flow cap.
...
- [x] **M37. Orchestrator engine — supervisory node, run-tree, delegation, success-gated task-DAG, swarm Layer-2** — shipped 2026-06-21 via `feature/orchestrator-engine` (merged to main FF `a6e82643→506c6876`; ADR-098/099/100, migration `0060_m37`, flow engine `1.6.0`). A supervisory `orchestrator` node type decomposes work into a child **run-tree** (`runs.parent_run_id`/`root_run_id`), drives it through a **delegation toolset** (`run_delegate`/`run_plan`/`run_collect`/`run_cancel`/`run_message`/`run_promote`/`run_rework` over MCP + `/api/v1/ext/runs/*`), gates a **`requires` success task-DAG** (a dependent releases only when its required task is `Done`; `Failed`/`Abandoned` keeps it blocked), and parks on **`WaitingOnChildren`** with idle-checkpoint wait/resume (excluded from the `MAISTER_MAX_CONCURRENT_AGENTS` cap, holds no scheduler slot). **Swarm Layer-2** (ADR-099): addressable persistent child sessions, star-routed messaging, per-agent read-only, and worktree allocation modes (`own` per-run | `shared` one pre-allocated tree per orchestrator with a serialized-writer guard). Delegated children settle to `Review`, then promote/rework via ADR-100 (`promoteChildRunForToken` forces `local_merge` + `autoOnReady`; rework `Review→Running` + `session/resume`). Adversarial-review hardening: `resolveActiveBoundRun` fail-closed on all 7 run-bound ext routes (a stale / terminal-tree token cannot delegate/promote/cancel) + token revoke in the abandon/stop/drop/crash cascade. **Follow-up (ADR-102) — shared-worktree tree-level review/promote**: re-enables `workspace_mode: shared` writable worktrees with a uniform child→`Review` finalize, a promote-time settled-gate, an idempotent tree-promote settling all shared siblings `Review→Done` exactly-once, reviewable tree-diff resolution, GC tree-awareness, and a finalize-relock defense against rework-during-merge — implemented + verified on `claude/priceless-goldstine-33d1f1` (4 commits, migration-free, no new `MaisterError` code; unmerged on its branch). Docs: `docs/system-analytics/orchestrator.md`; ADR-098/099/100/101.
- [x] **M38. Output-driven dynamic routing + run-context (P4 + P7, Wave 2)** — shipped 2026-06-22 via `feature/flow-routing-runcontext` (merged to main FF `e19d3e3f→293b6d75`; ADR-103, flow engine `1.6.0 → 1.7.0`, no migration). Continues the M26 keystone (P1 shipped): a node-level **`decide`** table routes on its own structured output (`from: output.<path>` — nested dot-path) or a gate/judge **`verdict`** (`from: verdict` + a closed `when` grammar, subsuming `confidence_min`); a schema-invalid output can enter a **bounded rework loop** via `output.result.on_mismatch` (`retry` self-sugar | an outcome routed to `rework.allowedTargets`, validation-error text injected into `commentsVar`) instead of hard-failing; and the **P7 run-context blackboard** (`<worktree>/.maister/run.json`, git-excluded, prompt pointer) flips M26 Designed → Implemented. `decide:{from:verdict}` makes the verdict gate routing-input (engine-owned, no `mode: advisory`). Plain (no-`decide`) routing stays byte-identical. Plan: `.ai-factory/plans/feature-flow-routing-runcontext.md`; extends spec `.ai-factory/specs/feature-m26-structured-output-run-context.md`; ADR-103.
- [x] **M39. Flow Studio package authoring** — Stream A (editor + first-class kinds, ADR-105, web-only, no migration) **landed 2026-06-23** on `feature/studio-package-authoring-editor` (commits `71095afb` A3 · `9f1413ec` A4-runtime · `3b3d046c` A4-kinds): the Studio **commit-time validation gate** (`validatePackageArtifacts`, hard-block on invalid changed flow/manifest/platform-agent/skill/subagent), **fork dedup** (200/201 + `alreadyExists`), project-less **"Customize"** (`<ref> (custom)`) + **element-fork** (`forkElementToNewLocal` → a new centralized package), **local-package list management** (delete/rename/archive/cut/open + archived toggle), the top-bar **Commit-state** action + shared **`ChangeReviewDialog`**, **create-package** affordance, the canonical **`maister-agents/`** platform-agent runtime convergence (registry/effective/lifecycle; root `agents/` retired, owner-accepted break), and the first-class **`subagent`** authored kind (lenient schema, classifier split, 8-site fan-out). The canonical Create Flow workflow is implemented on `feature/canonical-create-flow-journey`: one wizard creates a git-backed package with its initial valid Flow or adds another Flow to an editable package; it does not use the legacy DB-only authored-flow path. **ADR-106 package-based platform agents** (per-package re-key + per-agent runner policy + optional same-package flow enrichment) also landed on this branch with **migration 0068** (`0068_m39_package_agents.sql`). **Stream B — version-adopt launch + PR-to-source** (ADR-107 + ADR-113, **migrations 0074/0075**) **implemented 2026-06-25** on `feature/studio-package-authoring-runtime` (unmerged): the launch-time **keep / adopt / cut_and_adopt** prompt for a project's attached centralized packages (advance the pin via `upgradeAttachment` BEFORE the enablement check; `cut_and_adopt` mints a fresh cut through the Studio gate), the `package_installs.{source_local_package_id, source_commit_sha}` source-link provenance (derivable, **no `runs` column**), cross-project version reuse, run-detail provenance, and **PR-to-source** publish that pushes the package's committed HEAD to a registered `package_sources` target on a stable `maister/<slug>` branch (gh/glab/Gitea PR or push-only + compare URL, two-phase markers). Host E2E remains deferred. Plans: `.ai-factory/plans/feature-studio-package-authoring-editor.md` (Stream A) + `.ai-factory/plans/feature-studio-package-authoring-runtime.md` (Stream B).
- [x] **M40. Vendor-neutral guardrail/hook engine (G4)** — shipped
  2026-06-24 through ADR-108 and migration `0066`. Per-tool-call enforcement
  at the supervisor↔ACP seam runs `path_guard`, `repetition`, and
  `no_progress`; web checkpoint/escalation uses the dedicated `hook_trip` HITL
  and resumes per `run_kind`. The two-tier unattended default and the
  claude-only `NativeHookMaterializer` are implemented. The one residual is a
  live native-hook fires-and-denies confirmation; the universal supervisor
  path is covered by unit, integration, and E2E tests. See
  `docs/system-analytics/guardrail-hooks.md` and ADR-108.

- [x] **M41. Consensus flow-graph node** — shipped through ADR-109 and
  migration `0070`: engine-owned multi-participant draft verification,
  fail-closed verdict persistence, graph visualization, Studio authoring, and
  run/session integration.

- [x] **M42. Unified runner configuration and first-class run sessions** —
  shipped through ADR-114 and migration `0080`: `run_sessions` is the sole
...
- [x] **M45. Core-package process qualification on private projects** — prove
  ...
  process/project profile. Record time-to-first-success, platform-vs-package-
  vs-environment failure class, review reach, human-attention time, promotion
  outcome, exact package revision, engine range, runner profile, prerequisites,
  ...
  **Project Automations support (Implemented, ADR-139).** A project member may
  ...
  that converges crash recovery to one Run or one safe terminal outcome. This
...
- [x] **M46. Evaluation Foundation** — evolve the task-bound Experiment
  ...
  0107–0111; ADR-142..145; default `core:sdd-quality` method. No auto-promotion,
...
| M37. Orchestrator engine (run-tree, delegation, task-DAG, swarm L2 + ADR-102) | 2026-06-21 |
| M38. Output-driven routing + run-context                                     | 2026-06-22 |
| M39. Flow Studio package authoring                                           | 2026-06-25 |
| M40. Vendor-neutral guardrail/hook engine                                    | 2026-06-24 |
| M41. Consensus flow-graph node                                               | 2026-06-25 |
| M42. Unified runner configuration and first-class run sessions               | 2026-06-27 |
