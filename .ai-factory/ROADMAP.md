...
- [x] **M39. Flow Studio package authoring** — Stream A (editor + first-class kinds, ADR-105, web-only, no migration) **landed 2026-06-23** on `feature/studio-package-authoring-editor` (commits `71095afb` A3 · `9f1413ec` A4-runtime · `3b3d046c` A4-kinds): the Studio **commit-time validation gate** (`validatePackageArtifacts`, hard-block on invalid changed flow/manifest/platform-agent/skill/subagent), **fork dedup** (200/201 + `alreadyExists`), project-less **"Customize"** (`<ref> (custom)`) + **element-fork** (`forkElementToNewLocal` → a new centralized package), **local-package list management** (delete/rename/archive/cut/open + archived toggle), the top-bar **Commit-state** action + shared **`ChangeReviewDialog`**, **create-package** affordance, the canonical **`maister-agents/`** platform-agent runtime convergence (registry/effective/lifecycle; root `agents/` retired, owner-accepted break), and the first-class **`subagent`** authored kind (lenient schema, classifier split, 8-site fan-out). Gates green (tsc 0, unit 4751, agent-runtime integration ~96, eslint 0). **ADR-106 package-based platform agents** (per-package re-key + per-agent runner policy + optional same-package flow enrichment) also landed on this branch with **migration 0068** (`0068_m39_package_agents.sql`). **Stream B — version-adopt launch + PR-to-source** (ADR-107 + ADR-113, **migrations 0074/0075**) **implemented 2026-06-25** on `feature/studio-package-authoring-runtime` (unmerged): the launch-time **keep / adopt / cut_and_adopt** prompt for a project's attached centralized packages (advance the pin via `upgradeAttachment` BEFORE the enablement check; `cut_and_adopt` mints a fresh cut through the Studio gate), the `package_installs.{source_local_package_id, source_commit_sha}` source-link provenance (derivable, **no `runs` column**), cross-project version reuse, run-detail provenance, and **PR-to-source** publish that pushes the package's committed HEAD to a registered `package_sources` target on a stable `maister/<slug>` branch (gh/glab/Gitea PR or push-only + compare URL, two-phase markers). Gates green (tsc 0, version-adopt integration 10, publish unit 9 + integration 5, eslint 0, validate:docs:all, redocly, i18n parity). **Deferred** (Stream A polish): create wizards (#134) + host e2e. Plans: `.ai-factory/plans/feature-studio-package-authoring-editor.md` (Stream A) + `.ai-factory/plans/feature-studio-package-authoring-runtime.md` (Stream B).
- [x] **M40. Vendor-neutral guardrail/hook engine (G4)** — *Designed (SDD frozen 2026-06-23); implementation in progress on `feature/guardrail-hook-engine`.* Per-tool-call enforcement at the supervisor↔ACP seam — the safety primitive that makes unattended overnight loops safe (gates are post-node; the ADR-101 budget meters totals; neither catches a mid-node tool-call loop, an out-of-lane write, or a stall). A universal supervisor interceptor in `requestPermission` runs three rules — **`path_guard`** (deny-and-continue on a write outside `allowedPaths`, via the standardized ACP `toolCall.locations[].path` + kind-only fallback), **`repetition`** and **`no_progress`** (liveness breakers that halt → web checkpoint + escalate to `NeedsInput` + a dedicated **`hook_trip`** HITL, resumable per `run_kind`). Two-tier default: the breakers auto-arm under the `unattended` execution-policy preset (caps `MAISTER_HOOK_REPETITION_MAX=5` / `MAISTER_HOOK_NO_PROGRESS_TURNS=15`, per-node opt-out); `path_guard` is always opt-in. A claude-only `NativeHookMaterializer` (PreToolUse path-guard into `.claude/settings.local.json`) ships spike-gated with graceful degradation; rules 1 & 3 stay supervisor-only. Adds a 7th `hooks` capability class (engine `1.7.0 → 1.8.0`, `compat.engine_min ≥ 1.8.0`), migration `0066` (`hook_trip` in `hitl_requests.kind` + `assignments.action_kind`), and **no new `MaisterError` code** (a trip is recoverable, never `Failed`). Plan: `.ai-factory/plans/feature-guardrail-hook-engine.md`; spec `.ai-factory/specs/feature-guardrail-hook-engine.md`; domain doc `docs/system-analytics/guardrail-hooks.md`; ADR-108. (**M39** is reserved for the parallel Flow-Studio-package-authoring stream; G4 yields to M40 + migration `0066`, leaving `0062` for Studio.)
