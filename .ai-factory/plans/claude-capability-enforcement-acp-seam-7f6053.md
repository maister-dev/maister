...
| REQ-14 | Refuse a strict `tools`/`mcps` launch (`EXECUTOR_UNAVAILABLE`) unless the resolved adapter's `capabilityEnforcement` smoke `=== "ok"`. | AC-4 | `launch.enforcement-evidence.test.ts` (pending/error→refuse; ok→pass) · U · P3; `capability-enforcement.spec.ts` (gemini/opencode/mimo refuse) · E · P5 |
