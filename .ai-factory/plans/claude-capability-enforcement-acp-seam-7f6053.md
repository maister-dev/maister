...
| REQ-11 | Latch a `capability_guard` halt if a WRITE **executes** (`tool_call_update` status completed/failed) while still un-arbitrated — i.e. it never reached the seam (D5 sentinel). Keyed off EXECUTION, not the pre-permission streamed `tool_call`, so a legit write never false-halts (review blocker C1). | SDD-E | `guardrail-interceptor.integration.test.ts` (bypass write halts; arbitrated pending→permission→execution write does NOT halt) · I · P2 |
...
| REQ-17 | On resume **re-derive** the `enforcementProfile` (deterministic on the pinned inputs) and re-deliver it on the createSession body; the persisted `plan.enforcementProfile` is an AUDIT snapshot, never read back (as-built correction — the flow forces new-session per attempt). | AC-5, SDD-E | `enforcement-profile.test.ts` (re-derivation is byte-identical) · U · P3; `runner-agent-hooks.test.ts` (REQ-17: resume re-delivers the profile + resumeSessionId) · U · P5 |
...
| REQ-20 | Thread `enforcementProfile` `CreateSessionInput → createInput → StartSessionRequestSchema → SessionRecord → spawn seed`. | AC-1 | `types.test.ts` (schema accept/reject) · U · P2; `runner-agent-hooks.test.ts` (REQ-20: delivered on the createSession body; omitted/inert when absent) · U · P5 |
