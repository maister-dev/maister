...
6. Reconciliation is idempotent and always takes the mutation lease. It may
   finish an exact stage/final state, clear an already-completed state, or mark
   `recovery_required` on a missing/mismatched hash. If an initial-create row
   exists but its private stage has no atomically written journal and no final
   directory exists, recovery compensates only that private stage plus the
   unfinished row and returns `rolled_back`; the user restarts the wizard.
   The recovery action never adopts or removes a pre-existing final path. A
   recovery-required package is read-safe but blocks editor writes, Commit,
   Cut, archive, and delete until localized remediation resolves it. Existing
...
OpenAPI must document all three route shapes; 201 responses; recovery's 200
`rolled_back` outcome; 400 malformed private-journal response; and 401/403/404/409/422
semantics;
...
| `web/lib/local-packages/__tests__/service.integration.test.ts`: real Postgres + temporary filesystem/Git proof of package-plus-Flow initial commit, additional-Flow dirty state, duplicate/lock refusal, failure compensation, hash-proven replay, safe compensation of the DB-before-journal crash boundary, and fail-closed drift. `web/lib/local-packages/__tests__/git.test.ts` separately proves Git initialization uses `--no-verify` and does not run a hook. | `pnpm --filter maister-web exec vitest run --project integration -- 'lib/local-packages/__tests__/service.integration.test.ts'` plus `pnpm --filter maister-web exec vitest run --project unit -- 'lib/local-packages/__tests__/git.test.ts'`; the workspace includes the respective `lib/**/*.integration.test.ts` and `lib/**/__tests__/**/*.test.ts` paths. |
| `web/lib/local-packages/__tests__/create-flow-api-error.test.ts`, `web/app/api/studio/local-packages/__tests__/route.test.ts`, `web/app/api/studio/local-packages/[id]/flows/__tests__/route.test.ts`, and `web/app/api/studio/local-packages/[id]/creation-recovery/__tests__/route.test.ts`: exact create/recovery body-response contracts, localized safe Flow-refusal reasons, 400 malformed journal, 422 fields, URL/body identifier authority, lock/assistant/recovery refusal, and safe projection. | `pnpm --filter maister-web exec vitest run --project unit -- 'lib/local-packages/__tests__/create-flow-api-error.test.ts' 'app/api/studio/local-packages/__tests__/route.test.ts' 'app/api/studio/local-packages/[id]/flows/__tests__/route.test.ts' 'app/api/studio/local-packages/[id]/creation-recovery/__tests__/route.test.ts'`; unit includes both `lib/**/__tests__/**/*.test.ts` and `app/**/__tests__/**/*.test.ts`. |
| `web/components/studio/__tests__/create-flow-dialog.test.ts`, `local-packages-list.test.ts`, `packages-list.test.ts`, and page/editor tests: metadata wording, field-specific validation, fixed-package mode, empty/recovery CTA, active-member affordances, viewer read-only state, EN/RU parity, focus/error behavior, and no raw errors. | `pnpm --filter maister-web exec vitest run --project unit -- 'components/studio/__tests__/create-flow-dialog.test.ts' 'components/studio/__tests__/local-packages-list.test.ts' 'components/studio/packages-list.test.ts'`; unit includes both `components/**/__tests__/**/*.test.ts` and `components/**/*.test.ts`. |
...
| `web/e2e/studio-local-edit.spec.ts`: one browser happy-path journey creates package + Flow, opens its canvas, adds another Flow, changes the test Flow to all-`cli`, commits/cuts, attaches or repoints through the existing surface, launches from Board, and observes terminal evidence. Duplicate, localized-refusal, empty-package, and recovery boundaries stay in the narrower unit/integration rows above. | `pnpm --filter maister-web test:e2e -- e2e/studio-local-edit.spec.ts`; Playwright `testDir` is `web/e2e`, and `AUTHED_SPEC` explicitly matches `studio-local-edit.spec.ts`. It uses the real E2E Postgres DB and all-`cli` engine pattern, never the stale `forked-package-loop` `steps:` fixture. |
...
- [x] **T5.3 — Final completeness, consistency, and logical-hole review.**
  ...
  - Final validation commands:
    ...
    - `pnpm --filter maister-web exec vitest run --project unit -- 'lib/local-packages/__tests__/create-flow-contract.test.ts' 'lib/local-packages/__tests__/create-flow-api-error.test.ts' 'lib/local-packages/__tests__/git.test.ts' 'lib/db/__tests__/migration-journal-integrity.test.ts' 'app/api/studio/local-packages/__tests__/route.test.ts' 'app/api/studio/local-packages/[id]/flows/__tests__/route.test.ts' 'app/api/studio/local-packages/[id]/creation-recovery/__tests__/route.test.ts' 'components/studio/__tests__/create-flow-dialog.test.ts'`
...
| Durable recovery and filesystem/Git failure | Generated DB state records safe operation identity/hashes only. Any failure compensates files/row/journal only when ownership is proven; the initial DB-before-journal crash deletes only its private stage and unfinished row, returning localized `rolled_back` guidance to restart. Each later durable marker either completes an exact state, clears a proven compensation, or fails closed without overwriting drift. A recovery-required package stays visible but is never silently treated as ready. |
