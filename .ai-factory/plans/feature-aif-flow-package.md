...
- ✅ **Increment 2 — runtime** (`64cc9f41`, Reviewer SHIP, 3 unit tests): exported `runFormCollect(node, loaded, settings, {runtimeRoot, db})` + dispatch `case "form"` (`def.settings` type-safe `FormSettings`). Mirrors `runReviewHuman`/`runHumanStep`: resume → `{ok:true, vars:existing, needsInput:false}` (NO decision ⇒ outcome `success`, **does NOT unlink** — vars persist to `node_attempts.vars`, form has no rework re-entry; inline WHY comment + "unlink if on_reject ever added" note); first visit → `readAndValidateFormSchemaDoc(loaded.flowInstallPath, settings.form_schema)` + `validateFormSchemaVersion` + `needs-input.json` + `kind:"form"` HITL + `createHitlAssignmentForRun(actionKind:"form", roleRefs:settings.roles)` → `needsInput:true`. `FormSettings` type exported from config.schema. **`services/runs.ts:91` role-validation now covers form** (was human-only). Tested via mock-DB unit (mirrors `runner-graph.m17.test.ts`); dispatch wiring is typecheck-covered (consistent with how `runReviewHuman` is tested). **Full-graph traversal + `{{steps.intake.vars.*}}` interpolation deferred to T9 e2e.**
- ✅ **Increment 3 — UI + intake schema** (`0ab7e96`, typecheck clean; full unit suite green): `flow-graph-view.ts` gained a `"form"` `GraphNodeRole` (role `form` → label "Form"; cascade-safe — no exhaustive `Record<GraphNodeRole>`/switch, layout passes role through). Board form-HITL render in `hitl-decision-controls.tsx`: a `form` HITL whose stored schema has a non-empty `fields[]` renders per-field option **buttons + a free-text input** (pick-or-type), gated by the exported pure `formFieldsFromSchema`; any other schema (or non-form kind) falls back to the raw-JSON textarea — keeps the existing M17 P4 tests green. Confidence input suppressed in the structured-form branch (intake ≠ review; avoids `vars` pollution). Client wiring in `run-hitl-response.tsx`: `formValues` state + `handleFormFieldChange` + `submitForm` (per-field coercion by `field.type`, server re-validates). EN/RU i18n: `run.formInstructions`/`run.formCustomPlaceholder` + `workbench.graph.role.form` wired into the run-detail role map. **Response contract confirmed (no code change):** `handleFormHumanResponse` writes `responseToStore` → `input-${stepId}.json` (stepId = node.id), the exact field→value map `runFormCollect` reads as `vars`. `plugins/aif/flows/dev/schemas/intake.json` (`tests`/`logging`/`docs`) + a `formSchemaSchema`/`validateHitlResponse` guard test. **Decision: intake fields are `string`+`options`, NOT `enum`** — `enum` rejects values outside `options`, which would defeat design §6 "pick or type your own"; `string`+`options` surfaces the options as button hints while free-text still validates. **T4 COMPLETE.**
- 🟢 **RESOLVED in T7 (`105b5cd`):** the three suites quarantined in inc3 (`0ab7e96`) — `settings-fixtures.test.ts`, `package-authoring.test.ts`, `aif-manifest-m12.test.ts` — are fully restored against the authored `flows/dev/flow.yaml` (no `.skip` remains). They had targeted the OLD single-`plugins/aif/flow.yaml` flat layout removed by T1 (`26cedaaa`). Full unit suite now green: **2283 passed | 0 skipped | 0 failed**.
...
- ✅ **DONE (`105b5cd`, typecheck clean; full unit suite green 2283 passed / 0 skipped):** five graph manifests at `plugins/aif/flows/<name>/flow.yaml` — each with T3 `metadata` (title/summary/labels/route_when/links/sources per design §5), a `claude-code` runner_profile, and a `nodes[]` graph per design §9. `aif-dev` (10 nodes) carries typed artifacts (impl-diff requiredFor review+merge, aif-gate-result, ai-judgment), a blocking artifact_required(impl-diff) + advisory ai_judgment review gate, `verdict_calibration: 0.7`, engine_min 1.2.0; `aif-bugfix` engine 1.2.0 (impl-diff+patch); `aif-evolve`/`aif-roadmap`/`aif-init` engine 1.1.0. `checks` is an `ai_coding /aif-verify` node (stack-agnostic, no hardcoded pnpm). `commit` nodes gate on an **inline** `command_check` mirroring `capability/scripts/commit-gate.sh` (clean tree + Conventional Commits) — inlined because bundle `scripts/` are not materialized into the worktree (**T5 note resolved: inline, not script-ref**). Regression guard `web/lib/__tests__/aif-flows.test.ts` loads all 5 via `loadFlowManifest` (schema + graph validation); `validate-authored-flow` CLI reports valid / 0 issues for all 5. **The three suites quarantined in `0ab7e96` are RESTORED** (settings-fixtures + aif-manifest-m12 repointed to `flows/dev/flow.yaml`; package-authoring canonical test validates `flows/dev`) — **no `.skip` remains anywhere**. Remaining: T8 (dogfood wiring) + T9 (e2e) — need the live supervisor/dogfood environment.
