...
- **WIRING TODO (the remaining T6 work):** call both utils **once per run in `launchRun` (`web/lib/services/runs.ts`, right after `addWorktree` ~:571)** — NOT per-node, so the core graph runner is untouched. Query `capabilityImports` by `projectId` (status `Installed`) → for each, `copyBundleArtifactsToWorktree({installedPath: row.installedPath, worktreePath})`; then `writeAiFactoryConfigOverride({worktreePath, baseBranch: project.main_branch})`. Add a testcontainers integration test (seed a project + `capabilityImports` row + a fake bundle cache dir + a worktree; launch → assert `.claude/skills` copied with precedence + `.ai-factory/config.yaml` patched + skip-worktree set).
