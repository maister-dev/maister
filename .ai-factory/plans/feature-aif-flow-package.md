...
- 🟡 **QUARANTINED pre-existing reds (from T1/T2 restructure — NOT inc3, user-approved quarantine):** three suites targeted the OLD single-`plugins/aif/flow.yaml` flat layout that T1 (`26cedaaa`) removed → now skipped with a one-line reason + this plan reference: `settings-fixtures.test.ts` "launches the all-instruct aif flow" (`it.skip`), `package-authoring.test.ts` "imports the canonical AIF package artifacts" (`it.skip`), and `aif-manifest-m12.test.ts` "M12/M15 migration" (both `describe.skip` + guarded `beforeAll` so it collects-as-skipped instead of erroring). **T7 restores them:** repoint the all-instruct + M12/M15 artifact-contract assertions onto an authored `flows/<name>/flow.yaml`, and rewrite/remove the obsolete flat canonical-package import. Full unit suite is now **green** (2266 passed | 12 skipped, 0 failed) + typecheck clean.
