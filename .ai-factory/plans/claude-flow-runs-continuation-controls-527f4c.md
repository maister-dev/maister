...
- [x] **Task 29: Feature-B e2e + docs as-built + mandatory docs checkpoint.**
  ...
  **AC CONFORMANCE WALK (AC-B1..B13) — every AC names its green test.**
  ...
  | AC-B6 | `T-B6` — **both halves**: (store) `restart_node` closes the attempt `Reworked`/`operator_interrupt` and persists the correction as the HITL response; (retrieval) `loadPendingOperatorCorrection` returns it to the FIRST attempt started after the response, returns null once a second attempt exists (consume-once), and returns null for a node the correction does not target. The retrieval half was **added during `/aif-verify`** — the original walk cited T-B6 for an AC whose second clause ("but not in the following attempt") nothing asserted. Proven non-vacuous by neutralizing the consume-once guard and observing RED | integ |
