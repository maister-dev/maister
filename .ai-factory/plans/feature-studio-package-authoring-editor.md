...
- [x] **Canonical New Flow wizard** — implemented on `feature/canonical-create-flow-journey`: it creates a git-backed package with its initial valid Flow or adds another Flow to an editable package, using the existing editor, creation journal, commit/cut/attach lifecycle, and no DB-only authored-Flow path.
- [ ] Platform Agent / Subagent / Skill creation wizards + kind picker + templates; skill asset uploads via existing import.
...
- ★ **A4 PARTIAL** — subagent KIND + naming-split were verified, and Canonical Create Flow is now implemented. **REMAINING**: Platform Agent / Subagent / Skill wizards, the editor "create-affordance-when-dir-empty" polish (part of #133), and host E2E coverage for the broader per-kind creation set. Subagents remain creatable through the existing add-file affordance + `newSubagentTemplate`.
- ★★ **CURRENT POINTER** — Canonical Create Flow is no longer a deferred Task #5 item. Any follow-up should target only the unchecked non-Flow per-kind authoring and its host E2E coverage; retain the current canonical package-backed workflow rather than adding a separate authored-Flow path.
...
- [x] Final verify: **tsc 0, full unit 4751**, affected integration (local-pkg 27 + agent-runtime ~96) green, **validate:docs:all 272/272 mermaid + 560 ADR anchors**, **redocly: API valid** (4 pre-existing warnings, 0 errors), i18n en/ru parity, zero NEW `console.*`. **e2e → A1–A3 specs HOST-RUN GREEN 2026-06-23 (commit `9886612f`)** — diff/local-edit/package-viewer/import/ai-assistant, 5/6 clean + 1 retry; fixed a real editor crash + 5 spec bugs. Canonical Create Flow now has focused unit/integration coverage; host E2E remains a follow-up for the broader per-kind authoring set.
