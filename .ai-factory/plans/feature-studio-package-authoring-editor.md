...
- [ ] **Commit-state button + validating commit** (`validatePackageArtifacts`, hard-block invalid): prominent top-bar action + dirty indicator. NEW `web/lib/local-packages/validate.ts` ✅DONE+tsc-verified (UNWIRED — wire into commit route); all commit entry points route through it.
