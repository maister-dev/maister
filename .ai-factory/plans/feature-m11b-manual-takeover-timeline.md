...
| Failure-class row 5: process dies after the AFTER-side flip, before runner attaches → run stranded `Running` is RECOVERED by an idempotent takeover-return startup re-dispatch | `resume-recovery.test.ts::takeover-returned-stranded-running-is-requeued` | AC-8 | V7 |
