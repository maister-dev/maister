...
| `claim` 401/403 unauth / non-member (authz spec) | `takeover.integration.test.ts::claim-unauthorized-401-403` (unit `takeover.test.ts` proves it split: `401 when unauthenticated` + `403 when not a project member`) | AC-1 | V1 |
| `claim` 404 run not found / not visible (OpenAPI 0.6) | `takeover.test.ts::claim-run-not-found-404` | AC-1 | V1 |
| `return` 401/403 unauth / non-owner (OpenAPI 0.6) | `takeover.test.ts::401 when unauthenticated` + `takeover.test.ts::non-owner-return-403` | AC-1 | V7 |
...
| runner resumes at `transitions.takeover` (`checks`), staled gates rerun → fresh `human_review` | `takeover-resume.integration.test.ts::resume-reruns-staled-gates` | AC-4 | V4 |
...
| `HumanWorking` excluded from recovery sweep (survives restart, not `Crashed`) | `resume-recovery.integration.test.ts::humanworking-survives-restart` | (ADR-030) | V1 |
...
| Failure-class row 5: process dies after the AFTER-side flip, before runner attaches → run stranded `Running` is RECOVERED by an idempotent takeover-return startup re-dispatch | `resume-recovery.integration.test.ts::takeover-returned-stranded-running-is-requeued` | AC-8 | V7 |
...
> one owning row. Two release/abandon clauses are owned outside the takeover route
> tests by design: the `HumanWorking → NeedsInput` (release) and
> `HumanWorking → Abandoned` (abandon) run-state transitions are proven by the
> **Phase-2 `state-transitions` CAS-idempotency RED tests** (helpers
> `releaseHumanWorking` / `markAbandoned`), not by a route contract test.
> Failure-class row 5 (process dies after the AFTER-side flip, before the runner
> attaches → run stranded in `Running`) is owned by a **new Phase-2.6 RED test**
> `resume-recovery.integration.test.ts::takeover-returned-stranded-running-is-requeued`
> proving the idempotent takeover-return re-dispatch — it does NOT silently rely on
> M11a `runFlow` terminal precedence (which needs a live runner). No clause is
> unmapped.
...
| 2.6 (GREEN) | **(P6) Recovery + sweeper enumeration for `HumanWorking`.** Verify `web/lib/runs/resume-recovery.ts` (`runResumeRecoverySweep`, as-built selects `status='NeedsInput' AND acpSessionId IS NOT NULL`) does NOT classify a `HumanWorking` run (session-less by contract, holds a worktree) orphan→`Crashed` on startup — excluded by construction, but add an explicit guard + test; confirm the keepalive sweeper SELECTs (`web/lib/runs/keepalive-sweeper.ts`) do not sweep `HumanWorking` (it has no `keepalive_until` / is not `NeedsInputIdle`). Regression: a `HumanWorking` run survives a simulated restart WITHOUT flipping to `Crashed`. **(F3) ALSO add the takeover-return stranded-`Running` recovery candidate** (RED: `resume-recovery.integration.test.ts::takeover-returned-stranded-running-is-requeued`): a run in `Running` whose latest ledger activity is a recorded takeover return (takeover `node_attempts` row has `returned_diff`/`ended_at` set, re-entry `gate_results` still `stale`) AND with NO subsequent re-entry (`checks`) `node_attempts` row → **idempotently RE-DISPATCH the graph runner** at `runs.current_step_id` (the `transitions.takeover` re-entry). Relies on M11a's CAS-guarded resume (live runner → no-op; genuinely stale pointer → existing `runner-graph.ts:379` writes `Crashed`). **A naive `Running` + no-live-session → `Crashed` sweep is FORBIDDEN** — it false-positives on a session-less `command_check` gate running after the return. | `web/lib/runs/resume-recovery.ts`, `web/lib/runs/keepalive-sweeper.ts` | recovery skips `HumanWorking`; HumanWorking regression green; stranded-`Running` re-dispatch RED→GREEN; owns matrix row `takeover-returned-stranded-running-is-requeued` |
...
| Graph-runner resume kickoff fails after status flip (process dies after the AFTER-side flip, before the runner attaches) | 200 (return recorded) | `Running` | RECOVERED on startup by the **takeover-return recovery sweep** (Phase 2.6): an idempotent re-dispatch of the graph runner at `runs.current_step_id`, keyed on the recorded takeover return + still-`stale` re-entry gates + no re-entry (`checks`) attempt yet. NOT silently reliant on M11a `runFlow` terminal precedence (that needs a live runner). Naive `Running`→`Crashed` rejected (session-less gates). |
...
| 3.3 (GREEN) | Graph-runner resume after return: the runner resumes from `runs.current_step_id` (the takeover node), follows the node's `transitions.takeover` target (the declared validation re-entry — **`checks`** per P9, NOT `implement`); the staled downstream gates rerun over the human's commits before the next `human_review` HITL. Reuse M11a's resume/CAS machinery — NO new runner entry point, just a `HumanWorking`-aware resume gate alongside the existing `NeedsInput` resume gate. **(F3) Durability contract:** the return's AFTER-side `HumanWorking → Running` flip is covered by the **Phase-2.6 takeover-return stranded-`Running` recovery candidate** — a process death between the flip and the runner attaching is recovered on startup by the idempotent re-dispatch, NOT left stranded (CAS-guarded → the live re-dispatch here no-ops). | `web/lib/flows/graph/runner-graph.ts` (extend) | resume continues at the takeover node's `transitions.takeover` target; the re-entry node's PRIOR `gate_results` flip `stale` BEFORE the rerun creates fresh attempts; staled gates show `running`→fresh verdict; rerun recorded as new `node_attempts` rows; 3.0b `resume-reruns-staled-gates` GREEN; durability handed to Phase-2.6 sweep |
