...
- [x] **T19: as-built docs flip + validators + green gate** — Designed→Implemented
  flipped in task-queue/scheduler/runs/reconciliation-gc docs; `pnpm validate:docs`
  green; `pnpm typecheck` 0 errors; `pnpm test:unit` 5504 green; ADR-121
  integration suite (admission-gate, reconcile-stale-claim, auto-launch-triaged,
  scheduler-priority-promote, hitl, resume-recovery) green; external openapi redocly
  clean; web.openapi has only the 2 PRE-EXISTING `StudioAssistantActionResult`
  nullable-type-sibling errors (unrelated to ADR-121, quarantined). T18 renumber
  pass remains owner-gated at merge. — flip `Designed`→`Implemented`; `/aif-docs` checkpoint; `pnpm --filter maister-web lint` (scoped, NOT bare `eslint --fix` — memory); `pnpm test:unit && pnpm test:integration` green; quarantine any pre-existing red with tracked reason. **Doc/contract validators (Codex-2/3):** `pnpm validate:docs` (Mermaid) + `npx @redocly/cli lint docs/api/web.openapi.yaml` + `npx @redocly/cli lint docs/api/external/operations.openapi.yaml` (zero errors) + the 7-column schema-doc parity check (T2) + `scripts/validate-docs-adr-anchors.mjs`. AC: all validators pass; suite green; no stale `Designed` for shipped code.
