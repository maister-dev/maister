...
- [x] **T5.4: Falsification, then the final gate.**
  ...
  - **Final gate (2026-09-23, tree `00feda5a` / `ff84a6c8`; this Mac shared with other sessions' lanes, load 15–350).**
    ...
    - **Not green — web integration lane** (519 files, 6 failed / 5 files, 5 unhandled errors, load 45–70) **and the AB lane** (33 files, 29 failed / 2 files, 2 errors, load peak 220). Every red was classified; none traces to D4:
      - flows `prompt-owners` (27 AB reds; 15 in the idle re-run of 2df03018 at load ~350): **58/58 on BOTH HEAD and merge-base `ae99f9d0`** run concurrently at load 25–60; failures cluster in the load window, and the SIGKILL subset passes 6/6 alone.
      - `projection-worker` AT-03 (cursor did not reach 260): paired with the merge-base 3×, both 12/12, AT-03 32.2–33.1 s on either side — no throughput cost.
      - `permission-deadline` RED 13/14 (and RED 7 once): the host commits the interrupted prompt's rejection before `session.exited{checkpoint}` under load — a host-side teardown race D4 does not touch (no fast-feed settlement occurs in the file). Paired 5×: HEAD failed 2, 3, 0, 0, 0; merge-base 0, 1, 1, 0, 1 — **pre-existing on master**.
      - `run-transcript-projector`, `legacy-import-verify`: green alone (10/10, 13/13). `execution-ab-partitions`: green alone (6/6); one run right after the lane failed 6/6 on the fixture group the lane's failed P4 cleanup left behind.
      - Unhandled errors: `57P01` at teardown in untouched suites (dirty-resolution-race, observatory-cost ×2, consensus-prompt-owners ×2 in the AB lane — the file passes 7/7 alone with no error) → the teardown class tracked by chip `task_72ec42bc`; `ledger` "pool after end" (a late fake-host delivery) → chip `task_e325a623`; `reconcile-host-evidence` `57P01` (a D4 suite) → **fixed** `ff84a6c8` (drain the fake before `stop()`).
    - **Owner action before merge:** re-run the integration and AB lanes on a quiet host (memory baseline: zero failures on this Mac); this session could not get one.
