...
- Phase 0 `59f889230` — ADR-142..147 + migr 0107..0110 reserved; engine→3.2.0;
...
- Phase 2 T2.1 **partial** `6f88dd5f2` — migration **0107** (studies/recipes/
...
**T2.1 DONE (this session):** migrations `0108` (config: method_revisions /
judge_panels / profiles / project_profile_overrides), `0109` (execution +
evidence: 11 tables; `executions.method_revision_id` nullable for legacy;
`studies.legacy_snapshot` added), `0110` (retained idempotent parity-asserting
...
Migration numbering finalized: `0107` studies, `0108` config, `0109`
exec/evidence, `0110` legacy backfill, `0111` verdict-activity (T4.3),
`0112` M47 launch batches, `0113` M48 suites, `0114` M48 standardization;
**`0115+` reserved** for the deferred legacy-contract drop.
...
- ADR-147 — Advanced suites, calibration, and human-approved recipe standardization.
- 0104_evaluation_studies_expand (`0107_wet_red_skull`, Implemented).
- 0105_evaluation_platform_config (`0108_dizzy_speed`, Implemented).
- 0106_evaluation_execution_evidence (`0109_clear_major_mapleleaf`, Implemented; also adds `studies.legacy_snapshot`).
- 0110_evaluation_legacy_backfill (`0110_evaluation_legacy_backfill`, Implemented — the backfill was split out of 0107 into its own post-0109 data migration so it can synthesize legacy executions/attempts).
- 0111_evaluation_verdict_activity (`0111_evaluation_verdict_activity`, Implemented — T4.3; additive CHECK-widen adding the `evaluation_decided` task_activity event_kind for the social-board verdict mirror; never rejects existing rows).
- 0109_evaluation_launch_batches (`0112_happy_shiva`, Implemented — M47 T6.3; durable controlled-launch batch intent + per-item FSM tables).
- 0110_evaluation_suites (`0113_loud_champions`, Implemented — M48 T7.2; scheduled/regression suite parent + suite-round→Study link tables).
...
- evaluation_legacy_contract (RESERVED, unnumbered — 0115+; deferred contract/drop migration after the rollback window; NOT implemented in M46/M47/M48; 0112–0114 were claimed by the M47 batch tables, M48 suites, and M48 standardization ledger).
...
Migration 0107 must preserve or loudly refuse:
...
- [x] T0.1 Reserve global IDs and freeze product/domain decisions.
  - Deliverable (as shipped, commit `59f889230` + renumber `3e25f8c31`): rebase on current main; reserve/renumber ADR-142..147 and migrations 0107..0110; update .ai-factory/DESCRIPTION.md, .ai-factory/ROADMAP.md, and docs/decisions.md with terminology, JTBD, milestone boundaries, non-goals, ownership, invariants, and compatibility. REMAINING (not shipped under the lean freeze — co-evolves with the UI batch): the originally-planned docs/VISION.md, docs/PRODUCT_VIEW.md, docs/architecture.md, and .ai-factory/ARCHITECTURE.md updates were NOT made. Known in-flight sibling: branch claude/pr-merge-workflows-4d0e7e already claims ADR-139/140 and adjacent migration numbers — whichever integration lands second renumbers, including the migration journal `when` bump (non-monotonic `when` silently skips migrations).
...
- [~] T0.2 Freeze analytics, lifecycle, API, DB, package, evidence, judge, RBAC, and failure contracts.
  - SESSION DECISION (2026-07-16): LEAN FREEZE — core contracts frozen now (ADR-142..147, migration reservations 0107..0110, engine 3.2.0, milestone boundaries); the wider analytics/API/AsyncAPI/screens docs CO-EVOLVE with the code that implements them (per owner's "lean freeze, co-evolve" directive). Each later phase carries its own as-built doc checkpoint.
...
- [x] T2.1 RED/GREEN/REFACTOR migrations 0107–0110 and Drizzle schema.
  - DONE (0107): `evaluation_studies` / `evaluation_recipes` / `evaluation_participants` + `web/lib/evaluations/types.ts`; 7-test schema integration green.
  - DONE (0108/0109/0110, this session): config tables (`evaluation_method_revisions` / `evaluation_judge_panels` / `evaluation_profiles` / `evaluation_project_profile_overrides`); execution+evidence tables (11: snapshots/items/executions/objective_check_runs/metric_results/judge_attempts/criterion_results/aggregate_results/reviews/human_verdicts/events) with CAS `version`, score-state + zero-citation CHECKs, self-FKs (retry_of, supersedes), `executions.method_revision_id` NULLABLE for honest legacy executions, `studies.legacy_snapshot`; `0110` retained idempotent parity-asserting `evaluation_backfill_from_experiments()` (fixed status map, budget_restart→manual_relaunch, advisories→one Partial/`legacy_advisory` execution + attempt-per-advisory verbatim, advisory-less conclusions→zero-citation verdicts, RAISE+rollback on any mismatch). 10-test backfill integration green; journal-integrity(15) + drift-check(2) green; `validate:docs:all` green (367 mermaid). DB narrative + `docs/db/evaluations-domain.md` ERD added. **0111** = verdict-activity (T4.3); the deferred legacy-contract drop reservation is now **0115+** (0112–0114 were later claimed by M47/M48).
...
- [~] T5.2 RED/GREEN/REFACTOR full-page creation and N-way Study Lab.
  - DONE (core this session): `app/(app)/projects/[slug]/evaluations/{page,[studyId]/page}.tsx` (project-role-gated, ProjectTabs `evaluations` tab wired + `nav.evaluations`) + `components/evaluations/{study-list,study-lab}.tsx` + `lib/evaluations/lab-queries.ts` (`listStudyExecutions` aggregate scoreboard / `listEnabledProfiles` / `listComparableTaskRuns` flow-only). Study Lab covers the core loop: create Study (title+task) → N-way participants table + add-observed picker (flow runs, dedup) → Start evaluation (enabled-profile picker → `POST .../evaluations` start route + kick) → live SSE liveness pill + debounced RSC refresh over the durable event log → per-execution scoreboard (displayTotal + per-criterion + warnings) → human verdict panel (outcome + cite terminal executions + zero-citation ack + rationale, role-gated). Migrated-Study badge from `legacyExperimentId`. EN+RU `evaluationsLab` (58 keys, parity) + `nav.evaluations`. 4 `renderToStaticMarkup` client contract (member vs viewer, migrated badge, empty-state) + 3 lab-queries integration green; 171 unit across 28 files; typecheck baseline-only. CO-EVOLVE residual (Phase-5 polish / T5.3 E2E): pairwise diff picker + heatmap viz, task/run contextual entry-action buttons, legacy `/experiments/[id]` → Study redirect, URL-state deep-linking of the active tab/execution, materialization/trust preflight preview.
...
- [x] T6.1 Freeze and implement typed Evaluation Recipe/input-contract preflight.
  - DONE (core + service, this session): `lib/evaluations/recipe-schema.ts` (strict typed controlled-recipe zod for D16 — `flow` refs + input/artifact contract digests, `inputs` no-transform-script, `nodeAgentBindings`, `slotBindings` runner-override|typed-intent discriminated union reusing `flowRunnerConfigSchema`, `executionPolicy`/`capabilityOverlay`/`budgets` REUSED from existing platform schemas, `materializationIntent` declarative-only, `replicatePolicy`, FORCED `promotionHold.source=evaluation_study` literal); `lib/evaluations/recipe.ts` (`parseControlledRecipe` CONFIG-on-invalid + `computeInputContractDigest`/`computeArtifactContractDigest` for freeze-vs-live drift); `lib/evaluations/preflight.ts` (PURE `preflightControlledRecipe` — 15 typed refusal codes: ownership/trust/engine/schema/input-drift/artifact-drift/form-unknown/form-missing/artifact-uncovered/slot-unknown/slot-unbound/runner-unavailable/intent-unsatisfiable/overlay-unknown + soft-mismatch WARNING; aggregates ALL refusals, no side effects; EXACT compat, no coercion — M47 first scope); `lib/evaluations/recipes.ts` (`createControlledRecipe` typed-validated store + `preflightStudyRecipe` orchestrator with injectable contract loaders + study-ownership 404 guard). 23 unit (recipe-schema 8 + preflight 15) + 7 real-PG integration green; typecheck baseline-only. CO-EVOLVE (with T6.4 consuming UI): the live flow-manifest→`PreflightFlowRevision` projection adapter (slot keys / form fields / produced-artifact kinds / trust+engine facets) + the HTTP preflight route — the decision core + service are fully built+tested; only the live-catalog projection + HTTP surface land with the creation UI that consumes them (same co-evolve pattern as T2.2/T3.1 routes).
...
- [x] T6.2 Implement slot-keyed agent/runner/model bindings and immutable materialization recipe snapshots.
  - DONE (pure core, this session): `lib/evaluations/materialization.ts` — `resolveRecipeSlotBindings` resolves EVERY Flow-declared session/consensus slot against the recipe bindings (runner-override → capability-checked concrete pin; typed-intent → exact host runner OR recorded soft mismatch OR refusal; unbound-optional → default chain; unbound-required/stale/unavailable/capability-mismatch/unsatisfiable → 5 typed refusal codes), reusing `runnerIntentCandidates` from the platform resolver. INVARIANTS enforced: "no primary-session-only claim" (all declared slots resolved, aggregated), "never an unenforced modelId claim" (a soft mismatch persists the LAUNCHED model + requested-vs-launched delta, never the requested model). `buildMaterializationSnapshot` builds the immutable snapshot (resolved slots + capability overlay + materialization intent) with order-independent `slotsDigest`/`capabilityDigest`/`materializationDigest`. 12 unit green; typecheck baseline-only. CO-EVOLVE (with T6.3 launch): threading the resolved slot map through `launchRun`'s `ephemeralOverrides` seam into the actual `run_sessions` rows at launched-participant creation — the resolution + snapshot are built+tested; the launch-path insertion lands where launched participants are created (T6.3).
...
- [x] T6.3 Implement durable controlled batch intents, interaction policy, and launched-participant invariants.
  - DONE (durable batch core, this session): migration **0112** (`0112_happy_shiva`) — `evaluation_launch_batches` (status queued/launching/completed/partial/failed + nullable-UNIQUE idempotency key + CAS version) + `evaluation_launch_batch_items` (per-(batch,recipe,replicate) UNIQUE, status FSM queued→launching→launched|failed, run_id/participant_id/attempt/error_reason + CAS version). `lib/evaluations/launch-batch.ts` — `createControlledLaunchBatch` persists the intent BEFORE any side effect (D17), idempotency-dedups a double-click, validates every recipe belongs to the study + is not tombstoned + strict-parses the typed D16 definition (a legacy/opaque recipe cannot drive a controlled launch), refuses the whole batch on a bad item (no partial write); `runControlledLaunchBatch` drives queued items via CAS claim (queued→launching, crash-safe idempotent-skip on lost race) through an INJECTABLE `LaunchRunSeam` (mirrors T4.1 judge-spawn seam), creates each **launched** participant with launched lineage (recipeId + replicate ordinal + runIdentity) so `isLaunchedLineageRun` excludes it from auto-promotion/delivery, flips draft→open, writes launched|failed per item, and finalizes the batch status (completed|partial|failed); `retryFailedBatchItems` re-queues failed items bounded by maxAttempts. 7 real-PG integration green (durable intent + replicate expansion, idempotency dedup, cross-study reject, all-launched completes + launched participants + draft→open, partial batch per-item state + attempt count, crash-recovery no-double-launch, retry→re-drive→complete); migration journal-integrity(15)+CLI-contract(2)+drift(2) green; typecheck baseline-only. INTERACTION POLICY: the launched run carries the forced promotion hold via the seam's `autoPromote:false` (source=launch) AND the launched-lineage predicate (already migrated into all 4 no-auto/relaunch consumers in T2.2) — "auto-promotion exclusion at query and apply site" holds. CO-EVOLVE (with T6.4): the default `LaunchRunSeam` adapter wiring `launchRun({autoPromote:false})` + T6.2 slot resolution + ExecutionPolicy preview + the immediate-kick/scheduler-backstop arm — the durable FSM + launched-participant invariants are built+tested; the real run-launch adapter lands with the consuming UI (same seam discipline as T4.1). **★MIGRATION: 0112 is now the M47 batch tables (was the plan's 0112 legacy-drop reservation); the deferred legacy-contract drop reservation is now 0115+ (still NOT implemented in M47).**
...
- [~] T6.4 Complete controlled creation UX and materialization/trust/cost-scope preview.
  - DONE (preview surfaces, this session — the plan's named GREEN surfaces): `components/evaluations/controlled-preview.tsx` — `PreflightVerdict` (green all-clear / aggregated blocking refusals / advisory warnings, each localized by STABLE CODE not raw server message → "localized actionable errors") + `MaterializationPreview` (effective per-slot runner MODEL + capability with NO raw runner/package ids, soft-mismatch amber badge, capability-overlay change summary, execution-policy preset, always-held promotion, evidence method + coverage scope, estimated judge-attempt token footprint). Presentational (server-computed props in) → contract-testable. 6 `renderToStaticMarkup` tests + EN/RU `evaluationsControlled` namespace (preflight refusal/warning copy for all 15+1 codes, preview labels, policy presets) with i18n-parity green; typecheck baseline-only. CO-EVOLVE (with live create/preflight routes, themselves co-evolve): the full multi-step wizard (form steps 2–5, typed searchable selectors, graph slot editor, policy expander) that WRITES a recipe + calls the preflight route — the preview/verdict surfaces the GREEN criteria enumerate are built+tested; the write-path wizard lands with the live routes it drives. Live UI verification deferred per owner ("UI check will be performed later").
...
- [~] T6.5 Complete M47 integration/E2E/contracts and deploy behind Profile enablement.
  - DONE (integration/contracts + rollout gate, this session): rollout kill switch `controlledRecipesEnabled()` (`MAISTER_CONTROLLED_RECIPES_ENABLED`, default on) gating `createControlledLaunchBatch` — controlled/launched recipes disable independently of M46 observed comparison if a rollout degrades. Aggregating `m47-controlled.integration.test.ts` (5 real-PG): launched participant IS in the launched lineage (`isLaunchedLineageRun` true → no-auto-promotion/delivery at every consumer), a HYBRID study keeps its observed participant OUT of the lineage while both coexist, an incompatible recipe REFUSES at preflight with zero side effects (no batch/participant written — "before worktree"), a compatible exact-contract recipe passes, and the kill switch blocks new controlled launches while observed comparison still works. Combined with T6.1–T6.4 suites the M47 surface has: incompatible-refusal (T6.1 15 codes), N-slot resolution + soft-mismatch (T6.2 12), partial-batch recovery + dedup + crash-safety (T6.3 7), preview surfaces (T6.4 6). Full M47 slice green: 41 unit + 19 integration; typecheck baseline-only. CO-EVOLVE (owner-gated / deferred): live-app E2E playwright (UI check deferred per owner) + the deploy-behind-Profile-enablement rollout step (owner-gated, like T5.4). M46 regressions unaffected (observed path + launched-lineage predicate unchanged).
...
- [x] T7.1 Add pairwise and tournament Evaluation Methods without changing the Study/participant/evidence model.
  - DONE (this session): opened the CLOSED aggregation registry to `pairwise_tournament@1` + the `pairwise` mode (`method-schema.ts`), with a `.superRefine` COUPLING guard (tournament aggregation ⟺ pairwise mode — one without the other is a CONFIG refusal) and a `SCALAR_AGGREGATION_ALGORITHMS` split so the M46 scalar `combine` stays exhaustive (pairwise throws a defensive guard there — it never uses the per-criterion path). `lib/evaluations/aggregation/tournament.ts` (pure, client-safe) — `generateRoundRobinPairs` (stable all-play-all), `scheduleRoundRobin` (circle-method rounds + per-participant byes for odd fields), `resolveMatch` (a/b/tie plurality with conservative deterministic tie + below-quorum→`unresolved` incomplete-match, never guessed), `computeTournament` (win=1/tie=0.5 standings, standard-competition ranking with shared ranks on equal points+wins, byes folded without points, unresolved counted but standings-neutral, full provenance — exact match tallies carried). NO change to Study/participant/evidence tables (D-scope); `n_way` stays the default overview. 11 tournament + 5 new method (pairwise accept + both coupling refusals + registry membership) tests; full 135-test evaluations unit suite green (no regression from the registry/mode change); typecheck baseline-only. CO-EVOLVE (with pairwise UI, deferred per owner): the pairwise diff/bracket viz + the execution-routing that dispatches a pairwise-method execution through `computeTournament` instead of `computeAggregate` — the aggregation core + method contract are built+tested.
...
- [~] T7.2 Add scheduled suites, package-upgrade regression Studies, and calibration/longitudinal read models.
  - DONE (suite core + read model, this session): migration **0113** (`0113_loud_champions`) — `evaluation_suites` (versioned benchmark/regression PARENT OUTSIDE the one-task Study, kind scheduled|regression, digest-addressed definition, `last_trigger_revision`) + `evaluation_suite_studies` (immutable suite-round→generated-Study link, (suite,task,scanKey) UNIQUE dedup). `lib/evaluations/suites.ts` — `createSuite` (versioned, one-project/task-validated), `runEvaluationSuiteScan` (the dispatcher TICK the M24 scheduler calls — NO second clock: capped-scan per tick, generates ONE one-task Study per task via `createStudy` so every generated Study stays one project/task (D2), (suite,task,scanKey) idempotent dedup, POISON-SAFE per-task try/catch never aborts the scan, `regression` suites no-op on an unchanged trigger revision + scan on a change via injectable `resolveTrigger`), `computeSuiteLongitudinal` (calibration/drift read model over IMMUTABLE executions — per-scan-round study count + execution status distribution, PRIVATE-DATA-MINIMIZED: counts + status labels + scan key only, never evidence/prompts/bodies). 7 real-PG integration green (versioned create + cross-project reject, per-task generation + re-scan dedup, capped scan, disabled no-op, regression unchanged/changed trigger, longitudinal counts + minimization); migration journal-integrity(15)+drift(2) green; typecheck baseline-only. CO-EVOLVE (the ONE deferred seam — same split as T3.3's arm): registering the scheduler ARM on the M24 clock (job-catalog kind + budget + shared `claimDueJobs` CTE + tick-service case + seeded singleton) so `runEvaluationSuiteScan` fires on cadence — the tick FUNCTION + all suite logic + read model are built+tested; the clock registration is a mechanical follow-up deliberately deferred to avoid touching the shared scheduler CTE without a full scheduler-suite re-verify budget. **★MIGRATION: 0113 = M47/M48 boundary; the legacy-contract drop reservation is now 0115+ (still NOT implemented).**
...
- [x] T7.3 Add optional human-approved execution-recipe standardization.
  - DONE (this session): migration **0114** (`0114_bright_doctor_spectrum`) — `evaluation_standardized_recipes` (append-only project-default revision ledger, action standardize|rollback, source study/recipe/verdict provenance, copied immutable definition + digest, `rolled_back_to_revision`, (project,slot,revision) UNIQUE). `lib/evaluations/standardization.ts` — PHASE 1 `checkStandardizationEligible` (NO side effects; requires the conclusive latest-non-superseded `winner` verdict citing a LAUNCHED participant with a typed controlled recipe + a FRESH preflight over the LIVE contracts; every failure a stable code — `no_conclusive_winner`/`winner_not_launched_recipe`/`recipe_unavailable`/`preflight:<code>`), PHASE 2 `standardizeRecipe` (RE-checks eligibility inside the write tx so a dependency drifting between preview and confirm refuses; copies the winner recipe to a new project-default revision; append-only + audited actor/source; NO Run status change / no promotion — config default ONLY), `getCurrentStandardizedRecipe` (highest revision), `rollbackStandardization` (append a `rollback` revision restoring the prior definition; refuses with <2 revisions). Judge/token can never reach it (admin-gated at the route; the service is read + pure-check + config write). 8 real-PG integration green (eligible launched winner, no-verdict refusal, observed-winner refusal, incompatible-preflight refusal, standardize→revision bump, ineligible CONFLICT no-write, rollback restores prior digest, single-revision rollback refusal); journal-integrity(15)+drift(2) green; typecheck baseline-only. CO-EVOLVE (owner-gated / deferred per owner UI note): the admin preview/confirm/rollback UI + the HTTP routes (project-admin authz) — the two-phase eligibility + standardize + rollback + audit core is built+tested. **★MIGRATION: 0114 = M48 recipe standardization; the deferred legacy-contract drop reservation is now 0115+ (still NOT implemented).**
...
8. Contract migration: 0110 only after rollback window, parity queries, and legacy API/MCP sign-off.
