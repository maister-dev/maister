...
- 0109_evaluation_launch_batches (`0109_happy_shiva`, Implemented — M47 T6.3; durable controlled-launch batch intent + per-item FSM tables).
- 0110_evaluation_suites (`0110_loud_champions`, Implemented — M48 T7.2; scheduled/regression suite parent + suite-round→Study link tables).
- 0111_evaluation_standardized_recipes (`0111_bright_doctor_spectrum`, Implemented — M48 T7.3; append-only human-approved recipe-standardization audit ledger).
- 0112_evaluation_legacy_contract (RESERVED — deferred contract/drop migration after the rollback window; NOT implemented in M46/M47/M48; moved from 0111, which M48 T7.3 claimed for the standardization ledger).
