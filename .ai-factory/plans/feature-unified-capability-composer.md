...
- [ ] **T5.4 (GREEN)** Extend the send-path to structured content blocks end-to-end (scratch messages route → `supervisor-client` → supervisor request schema → `acp-client` prompt builder), preserving server-side secrets and verbatim-forward. **Path-confine `resource`/`resource_link` URIs to the worktree** (security, Rule: body-controlled path). **Reuse existing infra (don't build new):** `@file` lists via `GET /api/runs/[runId]/files` (running-scratch worktree) + `GET /api/projects/[slug]/files` (pre-launch/node-authoring → project repo; pre-launch has no worktree → project-files only); attachments via `web/lib/scratch-runs/attachments.ts` + the messages-route path (verify/extend to emit ACP `resource` blocks). Fan-out audit of all prompt builders.
  > FOUNDATION LANDED (Commit 6): TipTap deps (v3.26.1, MIT core only — `@tiptap/react`/`pm`/`core`/`extension-mention`/`suggestion`/`extension-document`/`-paragraph`/`-text`/`-placeholder`) + the pure canonical-token ⇄ segments serialize core (`composer-serialize.ts`, 7 unit tests). Note: the canonical→wire normalization at SEND already works for skill/agent chips via the Phase-1 `normalizeScratchPrompt` seam, so chips are functional end-to-end once authored.
  > DEFERRED (large, e2e-verification-blocked here): the interactive `CapabilityComposer` React component (T5.3), a `getProjectCapabilityCatalog` API route for the composer's static autocomplete, replacing the two textareas + EN/RU i18n, the content-block `@file`/attachment send-path + worktree path-confinement (T5.4), and the stub-supervisor playwright e2e (T5.2). The component's interactive behavior is **e2e-only verifiable by design** (node unit lane has no DOM) and the shared-infra e2e (ports 3100/7788 + shared `maister_e2e` DB) is infeasible/risky to run reliably in this headless context — so it is left for a focused follow-up session rather than shipped unverified.
