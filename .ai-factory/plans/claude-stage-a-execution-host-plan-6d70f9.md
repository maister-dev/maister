...
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| ADR                           | `git show main:docs/decisions.md` → max `### ADR-162` (main HEAD `73fa99915`); ADR-163 squatted by unmerged local branch `claude/flow-target-delegation-178968` | **ADR-166** (stub header written in T0.1 before any citation)      |
| Drizzle migration             | `_journal.json` max `idx 127` / `0127_output_contract`, snapshot present, `when` monotonic (last `1788257531779`); no local branch carries `0128+`              | **`0130_execution_hosts`** (single additive migration; Appendix C) |
| Engine version                | untouched                                                                                                                                                       | —                                                                  |
| Supervisor OpenAPI / AsyncAPI | both `v0.7.0`                                                                                                                                                   | bump to `v0.8.0`                                                   |
| Web OpenAPI                   | untouched (no new web route)                                                                                                                                    | —                                                                  |

T7.3 re-verifies at main HEAD after rebase; if ADR-163's branch dies first, ADR-166
...
- **Lazy assignment:** a command issuer meeting a NULL assignment calls
  ...
  deleted in Stage C (recorded in ADR-166).
...
| Errors                 | `docs/error-taxonomy.md`: blockquote "ADR-166 adds NO new `MaisterError` code" (`CONFLICT` + `details.reason`, `EXECUTOR_UNAVAILABLE`, `PRECONDITION` reuse) + supervisor code table `FENCED` (409) + reason-token table; `docs/supervisor.md` §Errors                                                                                                                                                               | D5      |
...
| ADR                    | `docs/decisions/adr-166.md` + stub + index; `docs/decisions/adr-023.md` amendment                                                                                                                                                                                                                                                                                                                                    | all     |
...
- **Commit 1** (T0.1–T0.5): `docs(execution-host): Stage A contracts — ADR-166, execution-hosts analytics, OpenAPI/AsyncAPI 0.8.0, ERD (Designed)`
- **Commit 2** (T1.1–T1.3): `feat(db): execution_hosts / execution_assignments / execution_commands + attribution columns (migration 0130)`
...
- [x] **T0.1 Reserve numbers; ADR-166; ADR-023 amendment.**
      Deliverables: `docs/decisions/adr-166.md` (Context = §Verified condensed;
...
- [x] **T1.1 Drizzle schema + migration triple + ERD.**
      ...
      `pnpm --filter maister-web db:generate` → `0130_execution_hosts.sql`; review
...
- [x] **T1.3 Migration behavior with historical + active runs.**
      ...
      RED (web `integration`, `web/lib/db/__tests__/migration-0130-execution-hosts.integration.test.ts`):
      M1 DB at `0127_output_contract` seeded with runs in all 11 statuses +
      `run_sessions` + `node_attempts` → `applyMainMigration("0130_execution_hosts")`
...
- [x] **T5.1 Supervisor strict flip + strict e2e test supervisor.**
      ...
      **Done (2026-09-03):** `types.ts`: `StartSessionRequestSchema` requires `executionWorkspaceId` and knows no path field (the former `superRefine` + `isHandleForm` gone; the residual `capabilityProfilePath` check lives ONLY in `resolveForSession` → W8); `legacySessionPathField()` names the first pre-ADR-166 field so the create route refuses it as `409 PRECONDITION {reason: legacy_field, field}` BEFORE the schema runs; `SessionListEntry`/`toSessionListEntry` = the `GET /sessions` projection (no `logPath`/`worktreePath`/`repoPath`/`confineRoot`/`contextMounts`; `+adapter, sessionName, acpSessionId, executionWorkspaceId, assignmentId, assignmentEpoch, createdByCommandId`); `SupervisorErrorDetails.field`. `http-api.ts`: `parseCommandBody` has no `allowLegacy` — a bare body is `missing_envelope` on EVERY command route (create/prompt/cancel/checkpoint/input/delete + adopt/release), `ParsedCommand.envelope` is non-null, `runCommand` always fences + writes receipts; `legacyResolution` deleted from `workspace-registry.ts`; `spawnSession` REQUIRES `workspace` (`legacyResolutionFromRequest` gone); `buildChildEnv` takes the `ChildEnvRequest` subset (the model-catalog probe + the smoke script build no synthetic session request). Z1 (bare create → `missing_envelope`, plus Z1b: every session command route), Z2 (each of the six legacy fields → `legacy_field` naming the field; the whole pre-ADR-166 shape → `legacy_field: runId`), Z3 (projection carries the handle + fence, no path key) in NEW `strict-envelope.integration.test.ts`. Z4 refined: the `SupervisorSessionRecord.logPath` field is gone and no web reader of a session record's `logPath` exists (`grep -rn "\.logPath" web/lib web/app` is empty; the remaining `logPath` identifiers are web-owned local path variables for the run-dir log files). Suites migrated to adopt-then-handle-form creates through the fixture's `adoptDirectory`/`createEnvelope`/`createSession` (ONE `directory` handle per host+run so fence choreography stays on the create; `bootHost.spawnOverrides` override; `_fixtures/workspace.ts` for spawn-level unit tests): lifecycle, command-receipts, execution-fence (F8 = missing envelope refused; F4 asserts the adoption fence untouched), workspace-adoption (W7 derives from the handle only), m8-resume-spike, model-application, guardrail-interceptor, ccr-e2e, mcp-forwarding, permission-roundtrip, cancel-route, checkpoint, types (+ `legacySessionPathField`), spawn, spawn-ccr, build-child-env, context-mounts (acceptor moved to `AdoptWorkspacePayloadSchema`), enforcement-profile, readonly-session, autoapprove-permission. E2E stubs: `stub-supervisor.ts` `stubResolveCreate` refuses `legacy_field` / a handle-less body, both stubs answer a bare create with `missing_envelope`, `test-supervisor.ts` `GET /sessions` drops `logPath`. Docs: `supervisor.openapi.yaml` (single `CreateSessionCommand`/`*Command` request schemas, `required: true` bodies, `StartSessionRequest` handle-only + `additionalProperties: false`, no deprecated props, `SessionRecord` without `logPath`) + `docs/supervisor.md`. Supervisor lanes: unit 403/403, integration 133/133; tsc + eslint clean; docs gates green.
- [x] **T5.2 Legacy backfill + lazy assignment.**
      ...
      **Done (2026-09-03):** NEW `lib/execution-host/legacy.ts` `adoptLegacyActiveRuns({db?, transport?, logger?, now?})` → `LegacyBackfillSummary {candidates, minted, leftNull, skipped: "no_host"|null, errors}`: candidates = `execution_assignment_id IS NULL AND status IN (Running, NeedsInput)`; the host is resolved through `localHost()` and `GET /sessions` read ONCE; a candidate with a live host session mints epoch 1 `legacy_backfill` under the run lock (re-checking NULL — a concurrent lazy mint wins), REUSING the `assignmentId` the host stamped on the session (later commands pass the host's own fence), copying the session's `executionWorkspaceId` onto the assignment and linking the `run_sessions` row; no session → left NULL (`legacy-backfill-left-null`, reconcile classifies); no registered/reachable host → `skipped: "no_host"`, WARN `legacy-backfill-skipped-no-host` once per outage (latch cleared when the host answers again), never throws. Wired into `instrumentation.ts` after `recoverExecutionCommands({graceMs: 0})` and into `executionCommandReconcilePass` (`ExecutionHostSweepSummary.legacy`), so a host unreachable at boot is picked up by the periodic sweep. Lazy path unchanged (`ensureAssignment` WARN `legacy-run-assigned-lazily` through `ExecutionHosts.forRun`). NEW `legacy-backfill.integration.test.ts` on the REAL supervisor: Y1 (Running + live session → `[1, active, legacy_backfill]` with the host's assignment id + handle, `runs`/`run_sessions` linked, idempotent), Y2 (Running without a session → NULL; reconcile re-drives the run whose worktree survived and Crashes the one whose worktree is gone — no backfill row for either), Y3 (NeedsInputIdle ignored; the resume claim mints `[1, released, resume, resume_rollback]` after a retryable spawn failure — placement, not backfill), Y5 (keep-alive sweeper on a legacy NeedsInput run → WARN `legacy-run-assigned-lazily`, `[1, legacy_backfill, checkpointed]`, NeedsInputIdle), Y4 (host down → `skipped: no_host` twice, ONE warn, no throw; the latch clears when the host is back). `docs/system-analytics/execution-hosts.md` legacy section documents the id reuse + sweep retry.
- [x] **T5.3 Remove legacy path fields from the web wire types.**
      Spec: E-EH-08.
      ...
      **Done (2026-09-03):** `CreateSessionInput` lost `runId, projectSlug, worktreePath, repoPath, confineRoot, contextMounts` (mounts ride `AdoptWorkspaceWirePayload`); `CreateSessionPayload = CreateSessionInput & {executionWorkspaceId}`; the bare `createSession(input)` wire function (the last path-bearing body builder) deleted — its unit tests ported to `createSessionEnveloped`; `SupervisorSessionRecord` mirrors the host projection (`logPath` gone; `+adapter, sessionName, executionWorkspaceId, assignmentId, assignmentEpoch, createdByCommandId`); every web fixture/e2e stub dropped `logPath`. Grep gate: `worktreePath` appears in neither `lib/supervisor-client.ts` nor `lib/execution-host/transports/` (the adoption payload is `AdoptWorkspaceWirePayload.path`). Residual, NOT removed (out of scope, mention only): the bare-body wire functions `sendPrompt`/`deleteSession`/`deleteSessionIfPresent`/`checkpointSession`/`cancelPrompt`/`deliverPermission`/`cancelPermission` remain exported for their unit tests and as route-test mock targets; no production path calls them (the transport uses the `*Enveloped` variants) and the strict host would answer them `missing_envelope`.

### Phase 6 — Deployment wiring + smoke

- [x] **T6.1 Deployment wiring (dedicated task).**
      ...
      **Done (2026-09-03):** `MAISTER_EXECUTION_HOST_STATE_DIR`, `MAISTER_EXECUTION_HOST_KEY`, `MAISTER_WORKSPACE_ROOTS` documented in `.env.example` (supervisor-only block next to `MAISTER_RUNTIME_ROOT`, mirror rule spelled out), `supervisor/.env.sample` (own "Execution host" section), `deploy/maister.env.example` (mirror note under the worktrees root), `web/.env.sample` (mirror comment under the supervisor URL); `docs/configuration.md` rows flipped to `Implemented`; `docs/deployment.md` §4 (Implemented), §10 smoke (+ the `execution-host-key-conflict` journal check = key-conflict remediation), §11 upgrade note (+ the sweep picks up a host unreachable at web boot, so the two services may restart in either order), linked-artifacts line; `docs/getting-started.md` wording. `web/playwright.live.config.ts`: the supervisor lane pins `MAISTER_EXECUTION_HOST_STATE_DIR` under `e2e/.runtime-live-supervisor/.maister/execution-host`, `MAISTER_EXECUTION_HOST_KEY=eh_e2e_live_supervisor`, `MAISTER_WORKSPACE_ROOTS=<lane worktrees root>`; `web/test-support/real-supervisor.ts` already passed the vitest lane root (T3.1). `scripts/blast-maister-local-state.mjs` now covers the execution-host state dir (`MAISTER_EXECUTION_HOST_STATE_DIR`, else `<MAISTER_RUNTIME_ROOT ?? cwd>/.maister/execution-host`) when it lies under a MAIster-owned root and SKIPS it with a note otherwise (never aborts the reset); `pnpm test:local-reset` 6/6 with the new case. Verified unchanged: `Dockerfile` (`mkdir -p /app/.maister` already holds the default state dir), `.dockerignore` (`.maister` excluded), `.gitignore` (`.maister/`), `compose.yml` / `compose.production.yml` (Postgres only). Docs gates green.
- [x] **T6.2 Live-lane smoke + authed e2e contract spec.**
      ...
      **Done (2026-09-03):** NEW authed `web/e2e/execution-host-contract.spec.ts` (registered in `AUTHED_SPEC`) on a NEW `executionHost` fixture (`E2E_EXECUTION_HOST_SLUG` = `e2e-execution-host`, one launchable project with a Backlog task bound to a single-`ai_coding` graph; `seedExecutionHostFixture` in `seed-e2e.ts`, type in `fixtures.ts`). The e2e test supervisor gained a **permission scenario** keyed on that slug: the first prompt of every session emits `session.permission_request` (events queue until the stream connects) and HOLDS its HTTP response; `/input` answers it `end_turn` and ends the session cleanly, `/checkpoint` answers it `cancelled` and emits `session.exited{reason: checkpoint}`, a resumed session re-issues the request, delete answers a held prompt `cancelled`. U1 — the board launch places the run: exactly one non-retired `local_direct` host (the stub's `hostKey`), `runs.execution_assignment_id` set, `[1, active, launch]`, a `ws_` handle on the assignment, `workspace.adopt` + `session.create` + `session.prompt` at epoch 1 on the ledger. U2 — `keepalive_until` forced past, the `system_sweep` job made due and ticked through `POST /api/cron/tick?jobKind=system_sweep` (the keep-alive sweeper lives inside it; no fallback timer in e2e) → NeedsInputIdle with `[1, released, launch]` + ONE `session.checkpoint`; `POST /api/runs/:id/hitl/:id/respond {optionId: allow}` → 202 → `[1, released, launch], [2, *, resume]` on the SAME `execution_workspace_id` (one adopt, two creates, the second at epoch 2 with `resumeSessionId`) → the resumed driver auto-delivers the stored answer → Review with every generation released. Passes in the e2e harness (12 s). Deviation: U1 places a FLOW launch, not a scratch one — the e2e scratch fixture's launch is capacity-refused by design (`scratch-launch.spec.ts` asserts that 409); scratch placement is pinned by Q1 (`scratch-placement.integration`) and by the live S2. Live lane: NEW `web/e2e/live-execution-host.spec.ts` (S1 `/health.host` identity = the registered `execution_hosts` row; S2 a live scratch launch → epoch-1 `launch` assignment with a `ws_` handle the host still answers on `GET /workspaces/:id`), `playwright.live.config.ts` `testMatch` widened to `/live-.*\.spec\.ts$/`, opt-in via `E2E_LIVE_SUPERVISOR=1` like the CCR spec. **`test:e2e:live` was NOT run on this machine — no live adapter/provider credentials are available here (only `ANTHROPIC_BASE_URL` is set); the owner runs it once with a configured adapter.** Full `test:e2e` (pre-T6.2 harness): 35 failed / 123 passed, the failing spec set ⊂ main's 35-spec baseline (`m27-flow-editor` passed this time).

### Phase 7 — Regression, as-built docs, renumber

- [x] **T7.1 Full regression pass.** Web unit + integration, supervisor unit +
      ...
      **Done (2026-09-03, on base `cd3ade544`):** supervisor unit 350/350 +
      integration 123/123; web unit 7284/7284 (2264 files); web integration
      3304/3339 — the 35 failures were exactly the 8 ADR-163 delegation suites
      main added after the fake host became a launch gate (503 "local execution
      host unavailable" / spy expectations); each got `fakeExecutionHosts(db)`
      (+ spy routing) → 68/68 in isolation, and no product file changed after
      that lane. `tsc --noEmit` + `eslint .` 0 errors in both packages;
      `pnpm validate:docs` green (88 mermaid, 341 ADR anchors, 874 links, 119
      indexed, ERD current); prettier applied to the 13 touched files that were
      clean on main (12 others were already unformatted on main — left). e2e
      (full authed lane, 10.4 min): 36 failed / 124 passed / 7 did-not-run —
      the failing spec FILE set (30) is IDENTICAL to the pre-rebase set. Two
      branch-caused regressions were found and fixed first: the rebuilt stub
      supervisor had lost main's ADR-163 `delegatedFlowRef` hunks (re-applied;
      `flow-target-delegation` green), and the contract spec's U2
      `system_sweep` tick also runs the workspace GC, which tombstones the m19
      fixture's past-deadline "due" workspace when it runs first →
      `m19-reconcile-gc (c)` now re-arms its two TTL rows (`removed_at` + the
      drop record) before reading the rail. Against main's baseline the only
      name-level delta is `m18-branch-promotion` "conflict scenario": its
      `toHaveCount(0)` on the text "Done" counts the left rail's hidden
      `status-tooltip` of the merge scenario's freshly promoted run, so it is
      coupled to that pre-existing merge-scenario failure through lane-level
      state (on main in isolation it passes even with the merge scenario
      first — the coupling was not fully isolated here); the rail and the
      promotion path are untouched by this branch, and main has since fixed the
      spec's isolation (`8ebc5a22f`, per-scenario target branches + scoped
      status assertions). Orphaned `mock-acp-lifecycle.mjs --hang` processes on
      the machine came from interrupted supervisor runs, not the fixture: the
      five hung-fixture suites run alone leak nothing (0 → 0 each).
- [x] **T7.2 As-built documentation checkpoint (mandatory).** Flip `Designed` →
      `Implemented` where true (sweep both forms: `(Designed …)` in prose and
      `| Designed |` in tables); ADR-166 `Implemented`; limitations list final;
      ...
      **Done (2026-09-03):** every `Designed` marker in a line this branch added (prose `(Designed — ADR-…)`, `**Designed — ADR-…**`, `(ADR-… — Designed, …)`, `(ADR-…, Designed)`, `ADR-… (Designed)`, table cells) flipped to `Implemented` across `CLAUDE.md`, `web/CLAUDE.md`, `.ai-factory/ARCHITECTURE.md`, `.ai-factory/rules/backend.md`, `docs/architecture.md`, `docs/supervisor.md`, `docs/configuration.md`, `docs/deployment.md`, `docs/getting-started.md`, `docs/database-schema.md`, `docs/db/*`, `docs/error-taxonomy.md`, `docs/system-analytics/*` (+ the `execution-hosts.md` header now reads as-built; E-EH-08 "strict since T5.1"), both AsyncAPI files, the supervisor OpenAPI, and the code comments in `web/lib/supervisor-client.ts` / `lib/execution-host/index.ts` (144 lines; the plan file itself is excluded — its task notes are history). ADR-166 body + `docs/decisions.md` index row and hub stub: `Implemented`. `CLAUDE.md` §Built-since gained the execution-host contract entry. Limitations kept as limitations: the multi-host grep finds only "single host / NOT supported / documented as a limitation" statements (`architecture.md` 74/434, `deployment.md` 533, `supervisor.md` 50, `execution-hosts.md` R-13). `docs/db/erd.dbml` is current (108 tables after main's sidecar drop). Docs gates green (88 mermaid blocks, 341 ADR anchors, 874 links, 164 hub stubs ↔ bodies).
- [x] **T7.3 Rebase + renumber pass (own session).** Rebase onto `main`; re-run
      preflight; renumber ADR/migration if squatted (grep prose forms `ADR-166`,
      `0130`, `pre-0130`); `git diff <backup> HEAD --quiet -- web/lib/db/migrations`
      proves migration bytes unchanged when no renumber was needed; full gates;
      **Commit 10**. Integration is rebase + FF by the owner.
      **Done (2026-09-03):** main had moved 37 commits (ADR-163 flow-target delegation + fixture repairs + the BREAKING managed-CCR removal) and squatted BOTH reserved numbers: main's `ADR-164` = "Remove managed Claude Code Router support" and `0128` = `0128_remove_router_sidecars`. The nine phase commits were rebased onto main `cd3ade544` one by one (backup ref `backup/stage-a-pre-rebase` = `4c7f78904`); conflicts resolved as main's text + this branch's additions (a diff3 union helper for pure-additive blocks, hand merges for the shared tables/routes; the e2e test supervisor rebuilt from the pre-rebase file with main's CCR removals re-applied). **Renumber:** this record is **ADR-166** (`docs/decisions/adr-166.md`; index row + hub stub; every `ADR-164`/`adr-164` in a line this branch added → 165, 531 lines) and the migration is **`0130_execution_hosts`** (journal idx 130, `when` 1788438913557 > main's 0128; `meta/0130_snapshot.json` = main's 0128 snapshot + this branch's 14 additions with `prevId` = main's 0128 id — a 3-way JSON merge, since the original snapshot pre-dated the sidecar drop; prose forms `migration 0128`/`(0128)`/`0128:` → 0129; the migration test file renamed). `git diff backup/stage-a-pre-rebase:…/0128_execution_hosts.sql HEAD:…/0130_execution_hosts.sql` is EMPTY — the SQL bytes are unchanged, only the number moved. **CCR removal absorbed:** `HostAdminClient` / transport / fake lost `startSidecar`/`stopSidecar` (+ the sidecar type re-exports); `buildChildEnv(request, { contextMounts? })` (no `ccrLayer`); `spawn-ccr` / `ccr-e2e` suites and the router-sidecar admin routes are gone with main; the live Playwright lane (main deleted it with CCR) was recreated CCR-free for `live-*.spec.ts` (`test:e2e:live` restored; S2 uses the `scratch` fixture). **Main's new code fenced:** `lib/runs/session-teardown.ts` + `lib/orchestrator/cascade.ts` (ADR-163) imported the wire client — ported to `ExecutionHosts` (`teardownLiveSessionsForRuns` lists ONCE through the admin client and deletes through each run's teardown-bound client; `CascadeAndStopOptions.executionHosts` threaded from reconcile + the budget sweeper; `reapAbandonedRunSessions.stopSession(record)` rides the fenced client). Owner note at the time: the unmerged recursive-agent-harness branch also claimed ADR-165 — it landed on main first (see the second-rebase addendum below), so this record moved on to ADR-166.
      **Second rebase (2026-09-03, later the same day):** main advanced 20 more commits before Commit 10 landed — the governed recursive agent harness merged as **ADR-165** with **`0129_run_results`** (engine 3.7.0, `runs.result_contract`, result-only completion), plus an m18 promotion-spec isolation fix (`8ebc5a22f`) and a recorded e2e baseline of 34 (`65dc6cd3c`). Both renumbered identifiers were squatted again, so Commit 10 was landed on the old base (`312b640be`, backup ref `backup/stage-a-pre-rebase-2`) and the ten commits were rebased onto main `65dc6cd3c`: commit 1 kept main's `adr-165.md` and moved this record to `docs/decisions/adr-166.md`; commit 2 kept main's `0129_snapshot.json` and wrote `0130_execution_hosts.sql` (bytes unchanged), `meta/0130_snapshot.json` (main's 0129 snapshot + this branch's 14 additive paths, `prevId` = main's 0129 id, no modified or conflicting keys) and journal idx 130 (`when` 1788444225214 > main's 0129); commits 6/7 placed `releaseAssignmentForRun` right after main's restructured terminal CAS guards (`runner-graph.ts` `failed_result_missing`, `agents/launch.ts` result publish); commit 9 merged the authed-spec regex (`recursive-harness` + `execution-host-contract`); commit 10's ADR hunks were re-targeted at `adr-166.md`. Post-rebase fix-ups (Commit 11): provenance-scoped renumber ADR-165 → **ADR-166** and `0129` → **`0130`** on 532 lines this branch added (main's ADR-165/0129 lines untouched — audited by listing every main line the tree removed; the one hub-stub link the union had rewritten was restored from main); the migration test became `migration-0130-execution-hosts.integration.test.ts`; two merge defects fixed (`e2e/_seed/fixtures.ts` had lost the `E2ERahFixture` closing brace; main's new `done_result_only` exit flipped Running → Done without releasing the assignment — `releaseAssignmentForRun(tx, runId, "run_terminal")` added, now 5 sites in `runner-graph.ts`); main's three harness suites (`run-result-flow`, `run-result-flow-terminal`, `agent-run-result`) ported from the removed `supervisorApi`/`api` seam to `fakeGraphHosts` / `fakeAgentExecution` (30/30; the two-attempt supersede test pushes per-session events from the prompt hook instead of swapping `streamSession`); ERD regenerated (109 tables). Gates on the rebased tree: supervisor unit 350/350 + integration 123/123; web unit 7481/7481 (2299 files); `tsc --noEmit` + `eslint .` 0 errors in both packages; `pnpm validate:docs` green (347 ADR anchors, 165 hub stubs in sync, 893 links, ERD current). e2e (full authed lane on the fixed tree, 9.4 min): 34 failed / 127 passed / 7 did-not-run, 0 flaky — 29 failing spec FILES, none new against the previous lane, the pre-second-rebase lane, or main's recorded 34-failure baseline (`m18-branch-promotion` dropped out thanks to main's isolation fix; `execution-host-contract`, `adr160-rework-claim`, `flow-target-delegation`, `m19-reconcile-gc`, and main's `recursive-harness` all pass). Before the Review-flip release was restored, the same lane had failed `execution-host-contract` (epoch 2 left `active` after Review) and `adr160-rework-claim` (the claim could not mint its epoch) — both verified green on main in isolation and green here after the fix. Web integration (full lane on the fixed tree): 3491 passed / 6 failed of 3497 (1343 files) — the 6 were main's two new harness suites that launch runs without a registered host (`delegate-result-profile` 503, `launch-run-result-contract` `local execution host unavailable`); both got `fakeExecutionHosts(db)` like the earlier delegation suites → 16/16, no other failure in the lane (`dirty-resolution-race` passed this time). **Router-removal + numbering audit (after the rebase):** the only file this branch re-creates that ADR-164 deleted is `web/playwright.live.config.ts` (router-free, for the execution-host live lane; `test:e2e:live` restored and loads/lists S1+S2 through `e2e/run.ts`); no router package returns in the lockfile (the supervisor only gains `yaml`); no `sidecar`/`ccr` identifier remains in this branch's code — three stale mentions were removed (a `boot-host` fixture comment, the live spec's comment, and a ledger-test payload that still carried the pre-removal `runner.sidecar` shape → `provider`). Numbered artifacts re-verified against main `65dc6cd3c` (= `origin/main` at the last successful fetch): ADR-166 (main max 165), migration 0130 (main max 0129, journal `when` monotonic, one snapshot added), supervisor OpenAPI + both AsyncAPI files 0.8.0 (main 0.7.0), engine 3.7.0 and milestones (M50) untouched.
...
## Appendix C — Migration `0130_execution_hosts` (authoritative; names declared in `schema.ts`)
...
- **2026-09-02 Phase 0 (T0.1–T0.5) — DONE, Commit 1.** Preflight re-run at
  ...
  `0128+` → ADR-166 + `0130_execution_hosts` confirmed. Gates:
...
execution_hosts` produced `0130_execution_hosts.sql` matching Appendix C
...
- **2026-09-02 Phase 2 supervisor substrate (T2.1–T2.5) — DONE, Commit 3.**
  ...
  ONE path-derivation site — `legacyResolution` reproduces the pre-ADR-166
...
code · 9 `MAISTER_WORKSPACE_ROOTS` defaults + git-root check · 10 ADR-166 +
