...
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| ADR                           | `git show main:docs/decisions.md` → max `### ADR-162` (main HEAD `73fa99915`); ADR-163 squatted by unmerged local branch `claude/flow-target-delegation-178968` | **ADR-165** (stub header written in T0.1 before any citation)      |
| Drizzle migration             | `_journal.json` max `idx 127` / `0127_output_contract`, snapshot present, `when` monotonic (last `1788257531779`); no local branch carries `0128+`              | **`0129_execution_hosts`** (single additive migration; Appendix C) |
| Engine version                | untouched                                                                                                                                                       | —                                                                  |
| Supervisor OpenAPI / AsyncAPI | both `v0.7.0`                                                                                                                                                   | bump to `v0.8.0`                                                   |
| Web OpenAPI                   | untouched (no new web route)                                                                                                                                    | —                                                                  |

T7.3 re-verifies at main HEAD after rebase; if ADR-163's branch dies first, ADR-165
...
- **Lazy assignment:** a command issuer meeting a NULL assignment calls
  ...
  deleted in Stage C (recorded in ADR-165).
...
| Errors                 | `docs/error-taxonomy.md`: blockquote "ADR-165 adds NO new `MaisterError` code" (`CONFLICT` + `details.reason`, `EXECUTOR_UNAVAILABLE`, `PRECONDITION` reuse) + supervisor code table `FENCED` (409) + reason-token table; `docs/supervisor.md` §Errors                                                                                                                                                               | D5      |
...
| ADR                    | `docs/decisions/adr-165.md` + stub + index; `docs/decisions/adr-023.md` amendment                                                                                                                                                                                                                                                                                                                                    | all     |
...
- **Commit 1** (T0.1–T0.5): `docs(execution-host): Stage A contracts — ADR-165, execution-hosts analytics, OpenAPI/AsyncAPI 0.8.0, ERD (Designed)`
- **Commit 2** (T1.1–T1.3): `feat(db): execution_hosts / execution_assignments / execution_commands + attribution columns (migration 0129)`
...
- [x] **T0.1 Reserve numbers; ADR-165; ADR-023 amendment.**
      Deliverables: `docs/decisions/adr-165.md` (Context = §Verified condensed;
...
- [x] **T1.1 Drizzle schema + migration triple + ERD.**
      ...
      `pnpm --filter maister-web db:generate` → `0129_execution_hosts.sql`; review
...
- [x] **T1.3 Migration behavior with historical + active runs.**
      ...
      RED (web `integration`, `web/lib/db/__tests__/migration-0129-execution-hosts.integration.test.ts`):
      M1 DB at `0127_output_contract` seeded with runs in all 11 statuses +
      `run_sessions` + `node_attempts` → `applyMainMigration("0129_execution_hosts")`
...
- [x] **T7.2 As-built documentation checkpoint (mandatory).** Flip `Designed` →
      ...
      `| Designed |` in tables); ADR-165 `Implemented`; limitations list final;
...
## Appendix C — Migration `0129_execution_hosts` (authoritative; names declared in `schema.ts`)
...
- **2026-09-02 Phase 0 (T0.1–T0.5) — DONE, Commit 1.** Preflight re-run at
  ...
  `0128+` → ADR-165 + `0129_execution_hosts` confirmed. Gates:
...
execution_hosts` produced `0129_execution_hosts.sql` matching Appendix C
...
- **2026-09-02 Phase 2 supervisor substrate (T2.1–T2.5) — DONE, Commit 3.**
  ...
  ONE path-derivation site — `legacyResolution` reproduces the pre-ADR-165
...
code · 9 `MAISTER_WORKSPACE_ROOTS` defaults + git-root check · 10 ADR-165 +
