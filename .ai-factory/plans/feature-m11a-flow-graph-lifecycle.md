...
| 3.4 | Rewire `runFlow` to dispatch: load manifest → `compileManifest` → `runGraph`. Linear flows traverse the compiled linear graph identically to today (behavioral parity) | `web/lib/flows/runner.ts` | `greet` + existing aif-suspend integration test pass unchanged |
