...
- Use the shared feedback provider for completed mutations and the shared portaled confirmation for destructive actions. UI action errors must resolve to EN/RU copy; never render a raw code, status, or server message. A busy confirmation disables Escape, backdrop, cancel, and repeat submit, then restores trigger focus after closure.
- Use the shared skeleton (`aria-busy`) and text-plus-color, `aria-live` liveness presentation for async routes and EventSource consumers. Liveness is presentation-only and never changes persisted run state. An error boundary may show only a recognized code in its localized diagnostic field.
