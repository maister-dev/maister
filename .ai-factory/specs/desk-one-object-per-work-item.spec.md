...
| **REQ-D11** | Narrow viewports MUST drop columns by priority instead of scrolling horizontally. The order, widest-first: `blockers` + `lastActivity` below `xl`; `readiness` + `waitingOn` + `tokens` below `lg`; `project` below `md`. A `<th>` and its `<td>` MUST carry the **same** drop class — a header that outlives its cells leaves every body row one column short of its own header from that point rightward, misaligning every label after it. Because hiding is CSS-driven the `<td>` elements remain in the DOM, so any `colSpan` MUST be the **full** column count, never the visible count. |
...
| **REQ-D14** | Panel content MUST resolve by stage: `WaitingOnHuman` → the HITL panel; `Review` → a **link** to the run's review surface, never an inline promote; `Crashed` → recover/discard; `Executing`/`Queued` → that run's recent events. **Amended 2026-09-17, post-review:** the `WaitingOnHuman` and `Crashed` arms resolve only when the reader's decision queue carries that run. The two read models this page joins are scoped differently on purpose — `getWorkTable` reads *visible* projects, `getDecisionsQueue` reads *actionable* ones (project `member` and up, ADR-169 D7 "actionable, not merely visible") — so a project-`viewer` sees those rows with no decision behind them. Such a row MUST render inert: no panel, and therefore no `aria-expanded`, no `tabIndex` and no hover affordance. Offering a reader an expansion whose every control answers 403 is the failure ADR-169 D7 already refused; a row that is simply not interactive is the correct degradation. The `Review` and `Executing`/`Queued` arms need no decision and stay available to every reader. |
...
| **AC-D11** | At 390px the low-priority columns are not visible, the page does not scroll horizontally, and the expanded row's cell still spans the full width; every `<th>` carries the same drop class as its `<td>`, so header and body stay the same width at every breakpoint | REQ-D11 | T-D11 — split by what each layer can see: the header↔cell PAIRING in unit/markup (3 cases; added 2026-09-17 after review found `waitingOn` shipped with the class on the cell and not the head, so the header outran the body by one column below `lg`), the `colSpan` VALUE in unit/jsdom (2 cases, incl. the conditionally-rendered project column), the painted width in e2e at 390px (jsdom applies no stylesheet) |
...
| **AC-D24** | For a seeded task that is simultaneously in flight and blocked on a human, the Desk renders exactly one object naming it; and the decision region is fed only the `flagged` kind, which no work row carries | REQ-D24 | `E2E-NAV-08` (e2e — id is m51-matrix-facing and must appear verbatim in the test title) **plus** a source-contract case in `desk-contract.test.ts` (added 2026-09-17 after review: the rendered consequence is only provable in e2e, which CI does not run, while the MECHANISM — the `kind === "flagged"` filter — is provable in the lane that does. `DecisionSections` buckets by kind internally, so widening that filter silently restores the duplication) |
