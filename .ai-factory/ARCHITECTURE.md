...
- **Project type:** Web control plane (Next.js + separate supervisor
  ...
  registered execution host behind a typed boundary (ADR-166) so later
...
- **Scale:** current target, single host (one registered local execution
  host; multi-host is a later stage behind the ADR-166 seam),
...
│       └── workspace-registry.ts   # (Implemented — ADR-166) POST /workspaces/adopt + handle resolution
│
└── web/                            # ── NEXT.JS WEB TIER ──
    ...
    │   ├── execution-host/         # (Implemented — ADR-166) registrar/resolver, assignments, command ledger+deliverer, adoption, recovery, BoundClient
...
- **Web ↔ supervisor:** HTTP + SSE only. `lib/execution-host/` is the
  single boundary (Implemented — ADR-166): domain code obtains a
