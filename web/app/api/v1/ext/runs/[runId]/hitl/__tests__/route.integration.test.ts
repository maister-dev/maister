...
// ADR-166: the permission delivery rides a `session.input` command through the
...
beforeAll(async () => {
  testDatabase = await startMainPostgresTestDb({
    databaseName: "ext_hitl_route_test",
  });

  pool = testDatabase.pool;
  db = testDatabase.db;
  // ADR-166: the resume claim mints on the local host — register a fake one.
