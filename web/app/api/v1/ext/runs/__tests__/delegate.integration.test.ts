...
beforeAll(async () => {
  ...
  testDatabase = await startMainPostgresTestDb({
    databaseName: "ext_delegate_test",
  });
...
beforeEach(async () => {
  ...
  // walks: an attached, trusted, Installed package_install pointing at the
  // agent .md root.
...
`,
  ...
  // (ADR-106) The package-anchored chain: an install carrying the given trust,
  // attached unless the package is "Disabled" — which in the package model
  // means NOT attached (attachment IS the enable; there is no enablementState).
  const untrustedInstallId = randomUUID();

  await pool.query(
    `INSERT INTO "package_installs"
       ("id", "source_url", "name", "version_label", "resolved_revision",
        "manifest", "manifest_digest", "installed_path", "package_status", "trust_status")
     VALUES ($1, 'github.com/acme/untrusted-pkg', $2, 'v1.0.0', 'rev-untrusted-pkg',
             '{}'::jsonb, 'digest', $3, 'Installed', $4)`,
    [
      untrustedInstallId,
      flowRefId,
      installedPath,
  ...
  if ((args.enablementState ?? "Enabled") !== "Disabled") {
    await pool.query(
      `INSERT INTO "project_package_attachments"
         ("id", "project_id", "package_install_id", "package_name")
       VALUES ($1, $2, $3, $4)`,
      [randomUUID(), projectId, untrustedInstallId, flowRefId],
    );
  }
