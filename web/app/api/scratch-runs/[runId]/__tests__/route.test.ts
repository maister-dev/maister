...
// M42 (ADR-114): the routes read runner identity + resume handle from the run's
// active run_sessions row; the fake DB models only `runs`, so derive the session
// from the run fixture (which still carries the runner fields).
