...
// T7.2 (ADR-147): the ONE seeded evaluation_suite_scan tick — drives every
// enabled Evaluation Suite through a capped scan round (budget 1, singleton).
// The (suite, task, scanKey) UNIQUE dedup keeps repeated ticks idempotent.
const DEFAULT_EVALUATION_SUITE_SCAN_JOB_ID = "evaluation_suite_scan.dispatcher";
const DEFAULT_EVALUATION_SUITE_SCAN_CADENCE_SECONDS = 60;
...
export async function ensureDefaultSchedulerJobs(
  ...
  await db.execute(sql`
    ...
    VALUES (
      ${DEFAULT_AUTO_LAUNCH_TRIAGED_JOB_ID},
      NULL,
      'auto_launch_triaged',
      '{}'::jsonb,
      ${DEFAULT_AUTO_LAUNCH_TRIAGED_CADENCE_SECONDS},
      ${now},
      3,
      ${now},
      ${now}
  ...
  await db.execute(sql`
    ...
    VALUES (
      ${DEFAULT_EVALUATION_SUITE_SCAN_JOB_ID},
      NULL,
      'evaluation_suite_scan',
      '{}'::jsonb,
      ${DEFAULT_EVALUATION_SUITE_SCAN_CADENCE_SECONDS},
      ${now},
      3,
      ${now},
      ${now}
...
export async function ensureRepoDeliveryScanJobs(
  ...
  await db.execute(sql`
    ...
    SELECT
      'repo_delivery_scan.' || p.id,
      p.id,
      'repo_delivery_scan',
      jsonb_build_object('projectId', p.id),
      ${REPO_DELIVERY_SCAN_CADENCE_SECONDS},
      ${now},
      3,
      ${now},
      ${now}
...
export async function disableArchivedRepoDeliveryScanJobs(
  input: EnsureDefaultSchedulerJobsInput = {},
): Promise<void> {
  const now = input.now ?? new Date();
  const db = input.db ?? (getDb() as unknown as SchedulerDb);
  ...
  await db.execute(sql`
    ...
    WHERE j.project_id = p.id
      AND j.job_kind = 'repo_delivery_scan'
  ...
  const result = await db.execute(sql`
    WITH budget_limits (budget_key, max_concurrent) AS (
      VALUES
        ...
        ('evaluation_suite_scan'::text, ${budgets.evaluationSuiteScan}::int)
    ),
    active_budget AS (
      SELECT
        CASE r.job_kind
          ...
          WHEN 'evaluation_suite_scan' THEN 'evaluation_suite_scan'
        END AS budget_key,
