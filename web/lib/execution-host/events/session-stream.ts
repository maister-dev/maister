...
import type { SupervisorEvent, SupervisorPermissionOption } from "@/lib/supervisor-client";
...
export function supervisorEventFromCanonicalRow(
  ...
  switch (row.eventType) {
    case "session.line": {
      ...
      return line === null ? null : { type: "session.line", sessionId, monotonicId, line };
    }
    case "session.update":
      return row.payload?.update === undefined
        ? null
        : { type: "session.update", sessionId, monotonicId, update: row.payload.update };
    case "session.permission_request": {
      const requestId = stringField(row.payload, "requestId");
      const options = optionsField(row.payload);
      if (requestId === null || options === null || row.payload?.toolCall === undefined) return null;
      return {
    ...
    case "session.exited": {
      ...
      if (
        exitCode === null ||
        (reason !== undefined && reason !== "checkpoint" && reason !== "intentional" && reason !== "fenced")
      ) return null;
      return { type: "session.exited", sessionId, monotonicId, exitCode, ...(reason ? { reason } : {}) };
    }
    case "session.crashed": {
      const exitCode = row.payload?.exitCode;
      const signal = row.payload?.signal;
      if ((exitCode !== null && !Number.isSafeInteger(exitCode)) || (signal !== null && typeof signal !== "string")) return null;
      return {
    ...
    case "session.chat_turn": {
      ...
      if (hitlRequestId === null || (role !== "user" && role !== "agent") || body === null) return null;
      return {
        ...
        body,
        ...(seq === null ? {} : { seq }),
        ...(typeof row.payload?.mutationReverted === "boolean" ? { mutationReverted: row.payload.mutationReverted } : {}),
      };
    }
    case "session.hook_trip": {
      ...
      if (
        (rule !== "path_guard" && rule !== "repetition" && rule !== "no_progress" && rule !== "capability_guard") ||
        (lifecycle !== "pre_tool_call" && lifecycle !== "post_turn") ||
        (disposition !== "deny" && disposition !== "halt") ||
        row.payload?.toolCall === undefined
      ) return null;
      return { type: "session.hook_trip", sessionId, monotonicId, rule, lifecycle, disposition, toolCall: row.payload.toolCall };
    }
    case "session.command": {
      ...
      if (
        commandId === null ||
        (kind !== "session.prompt" && kind !== "session.input" && kind !== "session.cancel" && kind !== "session.checkpoint" && kind !== "session.delete") ||
        (phase !== "accepted" && phase !== "completed") ||
        (status !== undefined && status !== "succeeded" && status !== "failed" && status !== "fenced")
      ) return null;
      const error = row.payload?.error;
      if (error !== undefined && (!error || typeof error !== "object" || Array.isArray(error))) return null;
      return {
        ...
        phase,
        ...(status ? { status } : {}),
        ...(row.payload?.result && typeof row.payload.result === "object" && !Array.isArray(row.payload.result) ? { result: row.payload.result as Record<string, unknown> } : {}),
        ...(error ? { error: error as { code: string; message: string; details?: Record<string, unknown> } } : {}),
      };
    }
    default:
      return null;
...
export async function* streamCanonicalSessionEvents(input: {
  ...
  while (!input.signal?.aborted) {
    ...
    for (const row of rows) {
      ...
      if (!event) continue;
      yield event;
      if (event.type === "session.exited" || event.type === "session.crashed") return;
    }
    if (rows.length === REPLAY_BATCH_SIZE) continue;
    await runEventWakeBus.wait(input.runId, WAKE_TIMEOUT_MS);
  }
}
