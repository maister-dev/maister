...
// ADR-164 D1 identity-change policy as DATA — first matching row wins. The
...
// ADR-164 D1: observe `GET /health`, then apply the identity-change policy
...
export async function ensureLocalExecutionHost(
  ...
  if (!health.identity) {
    return markUnavailable(
      db,
      "no_identity",
      "supervisor /health carries no execution-host identity (pre-ADR-164 supervisor)",
      now(),
      logger,
    );
  }
