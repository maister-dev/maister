...
export async function installRevision(opts: {
  ...
  try {
    ...
    // (ADR-106) Platform-agent registration moved OFF the per-flow revision
    // path: agents live at the PACKAGE root (`maister-agents/`), keyed by
    // package name, and are registered by installPackageRevision after the
    // member flow installs (see lib/packages/attach.ts).
