...
export const gateResults = pgTable(
  ...
  (t) => ({
    ...
    permissionResumeCheck: check(
      ...
      sql`(${t.permissionResume} IS NULL AND ${t.promptOrdinal} = 0) OR (jsonb_typeof(${t.permissionResume}) = 'object' AND ${t.permissionResume}->'version' = '1'::jsonb AND ${t.kind} IN ('ai_judgment', 'skill_check') AND ((${t.permissionResume}->>'kind' = 'permission' AND ${t.promptOrdinal} > 0) OR (${t.permissionResume}->>'kind' = 'permission_result' AND ${t.promptOrdinal} >= 0 AND jsonb_typeof(${t.permissionResume}->'inputCommandId') = 'string' AND jsonb_typeof(${t.permissionResume}->'checkpointCommandId') = 'string' AND ${t.permissionResume}->>'verdictSha256' ~ '^[a-f0-9]{64}$')) AND ${t.permissionResume}->'promptOrdinal' = to_jsonb(${t.promptOrdinal}) AND ${t.permissionResume}->>'parentActionSha256' ~ '^[a-f0-9]{64}$' AND jsonb_typeof(${t.permissionResume}->'sourceCommandId') = 'string' AND jsonb_typeof(${t.permissionResume}->'sourceAssignmentId') = 'string' AND jsonb_typeof(${t.permissionResume}->'sourceIncarnationId') = 'string' AND jsonb_typeof(${t.permissionResume}->'assignmentId') = 'string' AND jsonb_typeof(${t.permissionResume}->'resumeSessionId') = 'string' AND jsonb_typeof(${t.permissionResume}->'hitlRequestId') = 'string' AND jsonb_typeof(${t.permissionResume}->'sourceRequestId') = 'string' AND jsonb_typeof(${t.permissionResume}->'optionId') = 'string') IS TRUE`,
    ),
