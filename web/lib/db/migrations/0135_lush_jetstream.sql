...
BEGIN
  ...
  IF unresolved_run_id IS NOT NULL THEN
    RAISE EXCEPTION
      'Stage B cutover cannot begin: run % lacks five complete preservation records. Run `pnpm --filter maister-web execution-data-plane:import-legacy` against the legacy runtime mount and resolve any reported preservation failure before retrying migration.',
      unresolved_run_id;
  END IF;
