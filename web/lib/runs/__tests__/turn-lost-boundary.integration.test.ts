...
describe("applyTurnLostBoundary — the winner", () => {
  ...
  it("an ALREADY-discharged command still crashes — the obligation is met, not lost", async () => {
    ...
    // disagreeing turn nobody can see. Seeded with the quarantine that arm
    // writes: an `applied` command WITHOUT it is a successful application, and
    // the boundary yields to it (the case below).
    const seeded = await seed({
...
describe("applyTurnLostBoundary — every loser writes NOTHING", () => {
  ...
  it("a turn the host span settled after classification is not discharged over it (Codex review)", async () => {
    // Classified `stream-lost` while still `accepted` with no terminal
    // evidence; a concurrent host-span reader then settled it. Its result is
    // readable and the owner worker owes its application — the command's
    // `completion_applied_at IS NULL` guard alone would still have let the
    // boundary stamp it applied and crash the run over it.
    const seeded = await seed({
    ...
    await testDatabase.db
      ...
      .set({
        ...
        lastError: null,
        terminalEvidenceSha256: "e".repeat(64),
    ...
    expect(
      await boundary({
        runId: seeded.runId,
        commandId: seeded.commandId,
        expectedAttemptId: seeded.nodeAttemptId,
        reason: "stream-lost",
      }),
    ).toBe("lost-cas");
    expect((await read(runs, seeded.runId)).status).toBe("Running");
    ...
    expect(
      await read(executionCommands, seeded.commandId),
      "the settled result stays for its owner to apply",
    ).toMatchObject({ applicationState: "pending", completionAppliedAt: null });
  }, 60_000);
...
describe("applyTurnLostBoundary — the attempt's current turn", () => {
  ...
  it.each(["stream-lost", "turn-lost", "owner-poisoned"])(
    "a gate turn that APPLIED after its %s classification yields and writes nothing (Codex review)",
    async (reason) => {
      // The interleaving: the sweep classified the gate turn, then the gate
      // owner applied it (verdict recorded, command stamped applied). With the
      // action's completion admitted for a gate, an `applied` command read as
      // "already discharged" was the only thing left between a passed gate and
      // a crash. Only a quarantine stamped after application justifies it —
      // the case below.
      const seeded = await seed();
      const evaluationId = await gateEvaluation(
        seeded.runId,
      ...
      expect(
        await boundary({ runId: seeded.runId, commandId: gate, reason }),
      ).toBe("lost-cas");
      expect((await read(runs, seeded.runId)).status).toBe("Running");
      expect(await read(nodeAttempts, seeded.nodeAttemptId)).toMatchObject({
        status: "Running",
        endedAt: null,
      });
      expect((await read(schema.gateResults, evaluationId)).status).toBe(
  ...
  // The owner's apply locks run → assignment → session → attempt → gate (via
  // `lockFlowPromptOwner`); the boundary used to take the attempt (and a
  // gate's evaluation) first and reach the run only inside the crash. A second
  // connection replays the owner's order while the boundary waits on the run.
  it.each(["node", "gate_ai"] as const)(
    "takes the run lock first, as the owner does: a concurrent %s owner apply cannot deadlock it",
    async (variant) => {
      const seeded = await seed();
      const evaluationId =
        variant === "gate_ai"
          ? await gateEvaluation(seeded.runId, seeded.nodeAttemptId, "running")
          : null;
      const commandId = evaluationId
        ? await sibling(
            seeded,
            { variant: "gate_ai", gateId: "ci", evaluationId },
            `flow_node_attempt:gate_ai:${evaluationId}:0`,
        ...
        : seeded.commandId;
      const owner = await testDatabase.pool.connect();
      ...
      try {
        ...
        const crashing = boundary({ runId: seeded.runId, commandId });

        // Proceed only once the boundary is blocked on the run row the owner
        // holds — the moment the old order already held the attempt.
        await expect
          .poll(
            async () =>
              (
                await testDatabase.pool.query<{ n: number }>(
                  `SELECT count(*)::int AS n FROM pg_stat_activity
                   WHERE wait_event_type = 'Lock' AND pid <> $1
                     AND query ILIKE '%"runs"%'`,
                  [pid],
                )
              ).rows[0]!.n,
          ...
          .toBe(1);
        await owner.query(
          "SELECT id FROM node_attempts WHERE id = $1 FOR UPDATE",
          [seeded.nodeAttemptId],
        );
        if (evaluationId)
          await owner.query(
            "SELECT id FROM gate_results WHERE id = $1 FOR UPDATE",
            [evaluationId],
          );
        await owner.query("COMMIT");
        expect(await crashing).toBe("applied");
      } finally {
