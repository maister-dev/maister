...
// configured a default token ceiling, seed `tree.maxTokens` from the env var so
// a hands-off run is never unbounded by accident. Lives in a server module so
// the env read stays server-side and execution-policy.ts stays client-safe.
// Never throws — a missing / invalid / non-positive env value leaves the policy
// untouched (fail-OPEN, consistent with the budget axis as a whole).
