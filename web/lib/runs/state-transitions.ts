...
// NeedsInput → NeedsInputIdle CAS + the ADR-166 assignment release in ONE tx:
...
export async function rollbackResumeFromWait(
  ...
  const rolledBack: boolean = await db.transaction(async (tx: Db) => {
    ...
    // ADR-166: the generation minted by the wait-resume claim never drove.
...
export async function rollbackResumedRun(
  ...
  const rolledBack: boolean = await db.transaction(async (tx: Db) => {
    ...
    // ADR-166: the fresh epoch the resume claim minted never got a driver.
