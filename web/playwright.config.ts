...
const AUTHED_SPEC =
  /.*(m11[abc]-.*|m12-evidence-graph|m13-assignments|m15-.*|m16-.*|m17-.*|m18-.*|m19-.*|m22-.*|m23-.*|m27-.*|portfolio-board|task-launch-gating|project-registration|admin-users|project-members|review-comments|review-diff-scopes|gate-chat|scratch-launch|platform-acp-runners|model-suggestions|flows-authoring|run-schedules|flow-package-viewer|flow-studio-artifacts|outbound-webhooks)\.spec\.ts$/;
